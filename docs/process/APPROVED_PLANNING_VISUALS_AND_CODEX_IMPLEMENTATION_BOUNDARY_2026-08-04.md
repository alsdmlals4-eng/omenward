...
OMENWARD의 현재 작업은 **핵심 재미와 콘텐츠 기획을 먼저 완성하고, 그 재미를 UX·이미지·아트로 명확하게 전달**하는 데 집중한다. 실제 구현 방식은 Codex가 프로젝트 구조와 테스트를 확인한 뒤 결정한다.
...
```text
1. 이 게임을 계속 플레이하게 만드는 핵심 재미
2. 핵심 재미를 반복·변주하는 콘텐츠 구조
3. 플레이어가 판단하고 조작하는 UX
4. 세계·전장·유닛·건물의 이미지와 아트 표현
5. 구현에 필요한 결과 조건과 검수 기준
```

- 기술적으로 구현하기 쉬운가보다 플레이 경험이 선명한가를 먼저 판단한다.
- 이미지가 멋있는가보다 핵심 재미와 실제 게임 규칙을 설명하는가를 먼저 판단한다.
- 수치·알고리즘·자료구조는 핵심 재미나 콘텐츠 결과를 바꿀 때만 기획 Gate로 다룬다.
- 구현 세부 논의가 핵심 재미·콘텐츠·아트 논의를 밀어내면 중단하고 기획 우선순위로 복귀한다.

## 3. GPT가 정본으로 소유하는 것

- 무엇이 재미의 중심이며 반복 플레이의 원동력인지.
- 룰렛 설계·전선 배치·전투 복기가 어떤 선택과 보상을 만드는지.
- Stage·Wave·Boss·Danger·영웅·병종·건물 콘텐츠의 역할과 변주.
...
- 핵심 재미 또는 반복 플레이 동기.
- Stage·Wave·Boss·영웅·병종·건물의 역할과 콘텐츠 구조.
...
- GPT가 기술 세부를 핵심 재미·콘텐츠·이미지 논의보다 우선하지 않는다.
