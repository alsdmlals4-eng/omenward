# OMENWARD Asset Rights and Provenance Record
...
기능·구조·정보 위계·일반 형태·재질·주파수·타이밍·성능 원리만 분석한다. tracing, sample 재사용, mesh·texture·rig·animation clip·font glyph 추출, 특정 작가·성우 모사, 원본 AI 변환은 독립 제작으로 인정하지 않는다.

공개 저장소에는 unredacted 계약서, 신분증, 서명, 주소, 계좌·결제·세금·개인정보를 넣지 않는다. `secure_original_location`, 최소 metadata, hash와 적법한 redacted excerpt만 기록한다.
