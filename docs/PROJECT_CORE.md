...
```text
징조 관측
→ Stage 1: 사전 구축된 병영·농장·전진기지 방어탑의 역할 이해
...
Stage 1에는 플레이어 건설이 없다. Ward Citadel의 일반병 병영 1개·농장 1개, 각 Ward 전진기지의 방어탑 1개를 종류별로 설명한 뒤 첫 3×3 룰렛과 비가역 전선 커밋으로 넘어간다. 첫 실제 건설/업그레이드는 Stage 2의 T2 선택이다. 방어탑의 정확한 기능과 Veil 측 대칭 표기는 아직 결정하지 않는다.
