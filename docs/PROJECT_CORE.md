...
첫 세션은 실제 MapRun이며 실제 경제·전투 결과 규칙을 사용한다. Stage 1은 예고→첫 건설 선택→룰렛→병력 결과→비가역 배치→실전 전투→복기→첫 상인의 핵심 인과를 완성한다. Stage 2는 룰렛 통제와 다전선 판단, Stage 3은 마력탑·연구·수동 전술, Stage 4와 5는 학습한 시스템의 통합과 숙련을 시험한다.

첫 상인은 Stage 1 정비시간에 등장하며 선택 사항과 골드 기회비용만 가르친다. 벨루는 안내자이며 플레이어의 선택을 대신하지 않는다.
