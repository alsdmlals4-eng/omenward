...
첫 세션은 실제 MapRun이며 실제 경제·전투 결과 규칙을 사용한다. 기초 T1 건물은 이미 배치된 상태로 시작하며 역할은 짧게 확인한다. Stage 1은 예고→T1 빠른 확인→T2 업그레이드 판단→룰렛→병력 결과→비가역 배치→실전 전투→복기→첫 상인의 인과를 완성한다.

Stage 2는 룰렛 통제와 다전선 판단, Stage 3은 마력탑·연구·수동 전술, Stage 4와 5는 학습한 시스템의 통합과 숙련을 시험한다. 첫 상인은 선택 사항과 골드 기회비용만 가르치며 벨루는 플레이어의 선택을 대신하지 않는다.
