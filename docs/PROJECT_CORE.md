...
Stage 1에는 지도 건설이 없다. 전장에는 열린 지형, 세 shared front의 고정탑 1개씩, 유닛, 소유·접전 피드백만 보인다. 건물은 지도에 놓이지 않으며, Stage 2부터 `6 + 안정적으로 Lumern이 보유한 전진기지·접전지 수`의 전역 로스터 슬롯에서만 설치·업그레이드·우선순위 변경한다. 새 고정탑은 route state 보조 표현이며 단독 점령이나 전선 해결을 하지 않는다. 보이는 병영·농장·울타리·고정 바리케이드는 없다. 정확한 수치는 simulation 전 확정하지 않는다.
