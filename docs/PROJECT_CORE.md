...
Stage 1에는 플레이어 건설이 없다. Ward 본진의 고정탑 2개·잠긴 패드 4개, 각 Ward 전진기지의 바리케이드 1개·자동공격탑 1개·잠긴 패드 2개를 세 설명 단위로 읽은 뒤 첫 3×3 룰렛과 비가역 전선 커밋으로 넘어간다. 보이는 병영·농장은 없다. 첫 실제 건설/업그레이드는 Stage 2의 T2 선택이다. 안정 점령한 거점의 별도 건설 노드는 Stage 2 이후 해금된 승인 건물 중 하나를 설치하는 지도 장악 보상이며, 고정 방어 시설과 다르다. 정확한 수치는 simulation 전 확정하지 않는다.
