...
**Architecture:** `BattleFocusView` remains the only renderer of live combat units and the fixed tower, but it stops sampling the legacy three-front image and composes a user-locked neutral foundation with independent Lumern and Veil terrain props. Props are placed by horizontal territory thresholds so they can be freely rearranged, reused, or omitted. `RunCommandScreen` moves its three tabs into the top rail, freeing the full left battle area; `MarchMinimap` remains a read-only projection of the same `StageRun` route state.
...
### Task 1: Produce and review the composable visual candidates

**Files:**
- Create: `docs/images/candidates/OMENWARD_CLOSE_SINGLE_FRONT_BATTLEFIELD_FOUNDATION_2026-08-30.md`
- Create: `docs/images/candidates/OMENWARD_CLOSE_SINGLE_FRONT_LUMERN_TERRAIN_PROPS_2026-08-30.md`
- Create: `docs/images/candidates/OMENWARD_CLOSE_SINGLE_FRONT_VEIL_TERRAIN_PROPS_2026-08-30.md`
...
**Interfaces:**
- Produces: one neutral foundation source path/SHA/dimensions, three Lumern transparent prop paths, three Veil transparent prop paths, and one UI-reference candidate source path/SHA/dimensions.
- Does not produce: a runtime asset path, texture preload, scene binding, or altered game state.

- [ ] **Step 1: Generate the neutral terrain foundation candidate**

Use built-in ImageGen with the foundation requirements from the spec and no
image text. Confirm that the visual center is a broad continuous battle field
without foreground objects baked in.

- [ ] **Step 2: Generate six individual transparent terrain props**

Generate pale slab, blue-meadow-bank, and blue-flower-bank Lumern props
separately from charcoal rubble, violet crystal, and thorny-brush Veil props.
Require a genuine transparent background in each PNG and no building or tower
silhouette.

- [ ] **Step 3: Inspect against the map exclusions**

Reject the foundation if a river, bridge, road barrier, hard gap, baked
foreground prop, combatant, tower, building, UI, node, label, logo, or
watermark appears. Reject any prop that has a nontransparent background or
resembles a building, wall, tower, or construction node.
...
Record generator, source path, SHA-256, dimensions, alpha state, non-runtime
status, reference inputs, intended consumer, territory range, exclusions, and
approval boundary. Show all candidates to the user; do not copy any into
`assets/` before lock.

### Task 2: Bind the user-locked terrain with a RED-to-GREEN contract

**Files:**
- Create: `assets/art/battlefield/omenward_close_single_front_foundation_v1.png`
- Create: `assets/art/battlefield/props/omenward_lumern_low_slab_cluster_v1.png`
- Create: `assets/art/battlefield/props/omenward_lumern_meadow_bank_v1.png`
- Create: `assets/art/battlefield/props/omenward_lumern_blue_flower_bank_v1.png`
- Create: `assets/art/battlefield/props/omenward_veil_rubble_v1.png`
- Create: `assets/art/battlefield/props/omenward_veil_crystal_cluster_v1.png`
- Create: `assets/art/battlefield/props/omenward_veil_thorn_brush_v1.png`
...
- Produces: `BattleFocusView::BATTLEFIELD_FOUNDATION` and six terrain prop
  preloads; runtime placement uses `x_ratio` thresholds and no
  `ward_veil_three_lane_backdrop_v1.png` preload.
...
assert(source.contains("omenward_close_single_front_foundation_v1.png"))
assert(source.contains("omenward_lumern_low_slab_cluster_v1.png"))
...
Expected: failure because the legacy preload exists and the modular foundation,
props, and territory thresholds do not yet exist.
...
```powershell
git add assets/art/battlefield scripts/ui/battle_focus_view.gd tests/headless/close_battlefield_redesign_contract_test.gd docs/images/approved/OMENWARD_CLOSE_SINGLE_FRONT_BATTLEFIELD_V1.md
git commit -m "feat: bind composable close battlefield"
```
