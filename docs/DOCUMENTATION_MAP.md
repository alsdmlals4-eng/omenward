...
- 현재 단계: `R1_R2_PLANNING_REVIEW_IN_PROGRESS`
- 현재 제품 Issue: `#69`
...
| 입력 | 역할 | 권한 |
|---|---|---|
| Issue `#69` | 목표, 플레이어 가치, 포함·제외, Codex 제안서 완료 기준 | `CODEX_PLAN_MODE_INPUT` |
| `reviews/2026-07-26-v2-r1-r2-planning-review.md` | baseline·권한·고정 결정·벤치마크·UX 범위 검수 보정 | `REVIEW_AUTHORITY_FOR_CURRENT_PACKAGE` |
| `benchmarks/OMENWARD_V2_BENCHMARK_REFRESH_2026-07-26.md` | 유사 장르 비교, V2 UX 채택·후속·제외 판정 | `PLANNING_INPUT / NO_R1_R2_SCOPE_EXPANSION` |
...
→ R1+R2 구현 계획 초안
→ 사용자 정확한 `검수 완료`
→ 최종 Codex 읽기 전용 Plan Mode 인계
→ Codex 제안서 제출
→ 사용자 명시적 Build 승인
```

위 문서들은 제품 코드 승인 자체가 아니다.

## 4. 현재 검수 보정 규칙

- transient V2 runtime state는 `RefCounted`로 고정한다.
- token instance ID는 caller가 주입한다.
- R1+R2에서 global ID generator를 만들지 않는다.
- Codex는 위 결정을 다시 자유 선택하지 않고 실제 저장소와 Godot 4.7.1에서 성립하는지 검증한다.
- Codex 기준선은 실행 시점의 최신 `origin/main`이다.
- 계획 안의 오래된 고정 SHA는 조사 이력으로만 사용한다.
- 설계 청사진·전선 대응 브리핑·전투 인과 사슬은 후속 UX 패키지다.
- 설계 점검 구간은 강제 pause나 새 planning phase가 아니다.
- 런 청사진은 코어에서 맵 종료 로컬 요약만 의미한다.
...
| R1 중앙 판정 보존 seam | Issue `#69`, 현재 검수 보고서, 현재 R1+R2 계획, `design/APPROVED_ROULETTE_CORE_RULES.md`, Legacy C1 증거 |
| R2 물리 릴 순수 도메인 | Issue `#69`, 현재 검수 보고서, 현재 R1+R2 계획, 룰렛 책임 원본과 통합 결정 원장 |
| V2 장르 벤치마크·UX 적용 판정 | `benchmarks/OMENWARD_V2_BENCHMARK_REFRESH_2026-07-26.md` |
| Pre-V2 벤치마크 조사 이력 | `benchmarks/0001-core-game-benchmark-proposal.md`와 기존 benchmark 자료 — 역사·반응 조사만, 현재 구현 권한 없음 |
...
## 6. Base 공용 Skill과 프로젝트 Skill 경계
