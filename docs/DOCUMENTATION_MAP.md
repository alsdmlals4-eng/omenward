...
`최신 사용자 지시 → AGENTS.md → BASE_RULES_VERSION.md → DOCUMENTATION_MAP.md → GAME_DESIGN.md → 조건부 문서 → Issue/Goal → 대상 파일 → ACTIVE_CONTEXT.md`
...
| 실제 구현 시작 | 현재 GitHub Issue와 `docs/goals/`의 대응 Goal |
...
- 우선순위, Godot 불변 조건, GitHub 반영 방식, 완료 보고: `AGENTS.md`
...
- 외부 저장소 변경을 이 프로젝트에 자동 병합하지 않는다. 채택할 구조는 Issue/Goal과 프로젝트 문서에서 명시적으로 승인한다.