...
첫 상인은 Stage 1 정비시간에 등장하고 선택 사항·골드 기회비용만 설명한다. 정확한 T1 인스턴스 수·위치, 첫 T2 후보·최소 유효 경로·Danger/Boss 세부·실패 규칙·정확 시간은 `PENDING_GRILLME`다.
...
5/10 전술·마력 계보와 1~6/10 정본을 보존하며, 7/10은 실제 MapRun 형식, Stage 1~5 순서, T1 기본 배치와 T2·배치 첫 판단 경계를 부분 승인한다.
...
- T1 직접 건설 튜토리얼과 T1 장문 설명은 `[대체됨] / IMPLEMENTATION_INPUT_FORBIDDEN`이다.
