# [현행] OMENWARD 문서 수명주기 레지스트리

```yaml
updated_at: 2026-08-06
policy: OMW-PROC-20260804-DYNAMIC-CURRENT-MAIN-AND-DOCUMENT-LIFECYCLE-V1
process_policy: OMW-PROC-20260805-BENCHMARK-TDD-APPROVAL-BATCH-V1
status: CURRENT_LIFECYCLE_AUTHORITY
current_decision: OMW-DEC-20260805-PLANNING-FIRST-10-15-MINUTES-FLOW-V1
latest_child_decision: OMW-DEC-20260806-PLANNING-BUILDING-TIER-REALIGNMENT-V1
current_count: 7_OF_10_IN_PROGRESS
approval_checkpoint: PARTIAL_APPROVAL_6_OF_10
```

이 레지스트리는 파일명·과거 YAML·부분 문구보다 우선한다. `[대체됨]`, `[보류]`, `[폐기]` 문서는 신규 기획·Codex 구현·아트 제작 입력으로 사용하지 않는다.

## 1. [현행]

### 최상위·운영

- `PROJECT_CORE.md`
- `OMENWARD_GDD_CURRENT_CANON.md`
- `ACTIVE_CONTEXT.md`
- `CURRENT_IMPLEMENTATION_STATUS.md`
- `DOCUMENTATION_MAP.md`
- `process/APPROVED_DYNAMIC_CURRENT_MAIN_AND_DOCUMENT_LIFECYCLE_POLICY_2026-08-04.md`
- `process/APPROVED_BENCHMARK_TDD_AND_APPROVAL_BATCH_POLICY_2026-08-05.md`

### Planning Batch

- 1/10 `design/APPROVED_OMENWARD_CORE_FUN_AND_CONTENT_GUARDRAILS_2026-08-04.md`
- 2/10 `design/APPROVED_OMENWARD_STAGE_WAVE_DANGER_BOSS_PRESSURE_MATRIX_2026-08-04.md`
- 3/10 historical `design/APPROVED_OMENWARD_SIX_BUILDING_T2_T3_BRANCHES_AND_COUNTERS_2026-08-05.md`
- 4/10 `design/APPROVED_OMENWARD_TROOP_ROLES_SYNERGIES_AND_COUNTERS_2026-08-05.md`
- 5/10 `design/APPROVED_OMENWARD_TACTICAL_SKILLS_AND_MANA_2026-08-05.md`
- 6/10 `design/APPROVED_OMENWARD_STAGE_END_MERCHANT_2026-08-05.md`
- 7/10 partial `design/APPROVED_OMENWARD_FIRST_10_15_MINUTES_FLOW_2026-08-05.md`
- 7/10 checkpoint 6 `design/APPROVED_OMENWARD_BUILDING_TIER_REALIGNMENT_2026-08-06.md`
- 7/10 checkpoint 6 Spec `superpowers/specs/2026-08-06-building-tier-realignment-design.md`
- 7/10 checkpoint 6 Plan `superpowers/plans/2026-08-06-building-tier-realignment.md`
- 7/10 checkpoint 6 Review `reviews/ADVERSARIAL_BUILDING_TIER_REALIGNMENT_REVIEW_2026-08-06.md`

```text
OMW-DEC-20260805-PLANNING-FIRST-10-15-MINUTES-FLOW-V1
OMW-DEC-20260806-PLANNING-BUILDING-TIER-REALIGNMENT-V1
7_OF_10_IN_PROGRESS
PARTIAL_APPROVAL_6_OF_10
SYSTEM_EXPOSURE_ORDER = APPROVED_FOUNDATION_THEN_BRANCH_CHOICE
STAGE_1_T1_BUILDINGS = ONE_EACH_ALL_SIX
STAGE_1_REQUIRED_T1 = VAULT / FARM / GENERAL_BARRACKS / DEFENSE_TOWER / COMMAND_POST / MANA_TOWER
SPECIAL_BARRACKS_STAGE1_REQUIRED = FALSE
STAGE_1_T1_BUILD_BUDGET = GUARANTEED_SUFFICIENT_FOR_REQUIRED_SET
T1_PLACEMENT_POLICY = CATEGORY_COMPATIBLE_SAFE_NODES
T1_BUILD_ORDER = PLAYER_SELECTED
FOUNDATION_SETUP_RELOCATION = FREE_BEFORE_CONFIRMATION
FOUNDATION_SETUP_CONFIRMATION = REQUIRED
POST_CONFIRMATION_PLACEMENT_RULES = STANDARD_RUN_RULES
STAGE_1_REQUIRED_COST_RESERVE = SUM_OF_UNBUILT_REQUIRED_T1_COSTS
STAGE_1_NON_T1_SPENDING_BEFORE_REQUIRED_SET_COMPLETE = BLOCKED
T1_INVALID_PLACEMENT_TRANSACTION = ATOMIC_ROLLBACK_FULL_REFUND
FIRST_ROULETTE_UNLOCK = AFTER_ALL_SIX_T1_AND_SETUP_CONFIRMATION
FIRST_MEANINGFUL_BUILD_CHOICE = STAGE_2_T2_UPGRADE
STAGE_2_T2_CANDIDATES = TWO_RELEVANT_VALID_OPTIONS
```

### 현행 건물 Tier

- `design/APPROVED_OMENWARD_BUILDING_TIER_REALIGNMENT_2026-08-06.md`: 현재 건물 Tier 구조의 단일 책임 원본.

```text
GENERAL_T1_AUTO_PRODUCTION = BASIC_INFANTRY
GENERAL_T1_TOKEN_SOURCE = BASIC_INFANTRY
GENERAL_T2_BRANCHES = SHIELD / GREATSWORD / SPEAR / ARCHER / CAVALRY
GENERAL_T2_AUTO_PRODUCTION = SELECTED_GENERAL_UNIT
GENERAL_T2_TOKEN_SOURCE = SELECTED_GENERAL_UNIT

SPECIAL_T1_AUTO_PRODUCTION = RANDOM_SPECIAL_UNIT
SPECIAL_T1_TOKEN_SOURCE = NONE
SPECIAL_T2_BRANCHES = MAGE / PRIEST / ASSASSIN / FLYING_UNIT / GIANT
SPECIAL_T2_AUTO_PRODUCTION = SELECTED_SPECIAL_UNIT
SPECIAL_T2_TOKEN_SOURCE = SELECTED_SPECIAL_UNIT
SPECIAL_UNIT_FUNCTIONAL_POWER = STRONGER_THAN_GENERAL_UNIT
SPECIAL_AUTO_PRODUCTION_INTERVAL = LONGER_THAN_GENERAL_UNIT

DEFENSE_TOWER_T2 = ARTILLERY / DEFENSE_ENHANCEMENT / SNIPER
LINEAR_TIER_BUILDINGS = VAULT / FARM / COMMAND_POST / MANA_TOWER
LINEAR_T2_BRANCHING = FORBIDDEN
```

### 현행 시스템

- `design/APPROVED_VERTICAL_SLICE_SYSTEM_CONTRACT_2026-07-27.md`: 전체 시스템 연결 계보.
- `design/APPROVED_OMENWARD_COMBAT_HUD_ROULETTE_RESOURCE_MERCHANT_AND_BUILDING_ROSTER_2026-08-04.md`: HUD·룰렛·기본 건물 역할.
- `design/APPROVED_OMENWARD_BUILDING_TIER_REALIGNMENT_2026-08-06.md`: 건물 Tier 구조.
- `design/APPROVED_OMENWARD_TROOP_ROLES_SYNERGIES_AND_COUNTERS_2026-08-05.md`: 병종 대응망. 단, `전열/기동 병영 가중`은 부분 대체됨.
- `design/APPROVED_OMENWARD_TACTICAL_SKILLS_AND_MANA_2026-08-05.md`: 전술·마력. 단, `연사탑` 기반 건물 참조는 새 3분기 방어탑과 재검증한다.

### 플랫폼·출시

- `APPROVED_PC_ANDROID_PLATFORM_RELEASE_AUTHORITY_2026-08-05.md`
- `PLATFORM_RELEASE_AND_ASSET_RIGHTS_PROFILE.md`
- `ASSET_RIGHTS_AND_PROVENANCE_RECORD.md`
- `GAME_RELEASE_COMPLIANCE_EVIDENCE_PACK.md`

```text
OMW-DEC-20260805-PLATFORM-PC-ANDROID-V1
APPROVED_DUAL_PLATFORM
COMMON_PLATFORM_GATE
PC_RELEASE_GATE
MOBILE_RELEASE_GATE
RELEASE_BLOCKED_UNVERIFIED
```

## 2. [대체됨]

```text
OLD_BUILDING_BRANCH_CANON
source = OMW-DEC-20260805-PLANNING-SIX-BUILDING-T2-T3-BRANCHES-AND-COUNTERS-V1
status = SUPERSEDED / HISTORICAL_EVIDENCE_ONLY
replacement = OMW-DEC-20260806-PLANNING-BUILDING-TIER-REALIGNMENT-V1
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
UNIVERSAL_AB_BUILDING_BRANCH_GRAMMAR
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = per-building tier grammar
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_VAULT_BRANCHES
value = 안정 금고 / 행운 금고
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = 금고 직선 Tier 강화
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_FARM_BRANCHES
value = 징집 농장 / 예비 농장
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = 농장 직선 Tier 강화
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_BARRACKS_BRANCHES
value = 전열 병영 / 기동 병영
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = 일반병 병영 5분기 + 특수병 병영 5분기
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_DEFENSE_TOWER_TWO_BRANCHES
value = 연사탑 / 포격탑
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = 포격탑 / 방어 강화형 / 저격탑
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_COMMAND_POST_BRANCHES
value = 돌격 지휘소 / 수비 지휘소
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = 지휘소 직선 Tier 강화
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_TERM_MASOK
status = SUPERSEDED_TERMINOLOGY
replacement = 마력
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_MANA_TOWER_BRANCHES
status = SUPERSEDED_BY_DECISION_5_OF_10
replacement = 마력탑 T1 → T2 → T3 / one active instance / no branch
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_ALWAYS_AVAILABLE_SHOP
status = SUPERSEDED_BY_DECISION_6_OF_10
replacement = Stage 1~19 종료 정비시간 방문 / Stage 20 상인 금지
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_INFINITE_MERCHANT_STOCK
status = SUPERSEDED_BY_DECISION_6_OF_10
replacement = four finite visit slots / no infinite purchase or reroll
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_DIRECT_CORE_REWARD_SALES
status = SUPERSEDED_BY_DECISION_6_OF_10
replacement = repair/research/roulette/build discount support only
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_SEPARATE_TUTORIAL
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = real MapRun in-run progressive disclosure
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_STAGE1_FULL_SYSTEM_DUMP
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = Stage 1 foundation setup, Stage 2 T2 choice, Stage 3 tactical chain
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_SCRIPTED_TUTORIAL_VICTORY
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = real economy and combat result rules
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
SUPERSEDED_PREBUILT_T1_START
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = Stage 1 player builds one each of six required T1 buildings
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
LEGACY_LONG_T1_BUILDING_EXPLANATION
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = BRIEF_ROLE_LABELS_AND_RECHECKABLE_TOOLTIPS
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
UNSAFE_UNRESERVED_STAGE1_SPENDING
status = SUPERSEDED_BY_DECISION_7_OF_10_PARTIAL_6
replacement = remaining required T1 cost reserve and non-T1 spending block
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
FREE_RELOCATION_AFTER_CONFIRMATION
status = FORBIDDEN_BY_DECISION_7_OF_10_PARTIAL_6
replacement = free relocation only before foundation setup confirmation
IMPLEMENTATION_INPUT_FORBIDDEN
```

```text
PARTIAL_BUILD_TRANSACTION_COMMIT
status = FORBIDDEN_BY_DECISION_7_OF_10_PARTIAL_6
replacement = atomic rollback and full refund
IMPLEMENTATION_INPUT_FORBIDDEN
```

기타 대체 문서:

- `OMENWARD_GAME_DESIGN.md`: `OMENWARD_GDD_CURRENT_CANON.md`이 승계.
- `design/APPROVED_15_WAVE_STAGE_CLOCK_AND_OVERTIME_V2.md`: 20 Stage 정본으로 대체.
- 과거 post-merge Sync 문서: 당시 증거만 보존.

## 3. [보류]

- `T1_EXACT_NODE_COORDINATES`: 전장 레이아웃 검증 전 구현 입력 금지.
- `FIRST_T2_UPGRADE_CANDIDATE_IDENTITIES`, `FIRST_STAGE2_T2_CANDIDATES`, `STAGE_2_LEFTOVER_GOLD_POLICY`, `MINIMUM_VALID_PATHS`, `BELU_INTERVENTION_LEVEL`, `DANGER_EXACT_PRESSURE`, `BOSS_EXACT_PATTERN`, `FAILURE_RETRY_SKIP_RULES`: 후속 GrillMe 전 구현 입력 금지.
- `GENERAL_AND_SPECIAL_EXACT_PRODUCTION_INTERVALS`, `TOKEN_SOURCE_WEIGHT_AND_COUNT`, `T2_EXACT_COSTS`: 시뮬레이션 전 확정 금지.
- `SPECIAL_T1_RANDOM_SELECTION_TIMING`, `SPECIAL_T1_RESULT_PREVIEW`, `T3_IDENTITIES_AND_EFFECTS`: 후속 설계 승인 전 구현 입력 금지.
- Hero·Legendary 문서군: 8/10 재조정 전 구현 입력 금지.
- Meta·Hub 문서군: 9/10 재조정 전 구현 입력 금지.
- 구형 구현 계획: 재실행 금지, 과거 결정·검증 증거로만 사용.

## 4. [폐기]

- 식량을 현행 핵심 HUD 자원으로 사용.
- 기본 건물 5종.
- 지휘소 주변 범위 오라.
- `15웨이브=1스테이지`·고정 60초.
- Stage 중 숨은 필수 카운터 변경.
- 룰렛 전용 상징 아이콘과 T3 병종 룰렛 토큰.
- 병종 보유량 기반 기본 세트 보너스.
- 반대 병영 계열 영구 삭제.
- 특정 병종·전술·상인 상품 미보유 시 통과 불가능한 단일 하드키.
- Stage 전 전술 편성 슬롯.
- 자동 전술 시전·자동 대상 확정.
- 연구에 마력 소비.
- 마력탑 복수 활성·병렬 연구.
- T3 전술의 부활·완전 회복·전면 정지·전선 자유 이동.
- 상시 접근 상점·무한 재고·무한 새로고침·할인 중첩.
- 상인의 병종·전술·마력·분기 직접 판매.

## 5. [증거]

```text
[증거] data/units/*.tres
status = LEGACY_PROTOTYPE_UNIT_DATA
IMPLEMENTATION_INPUT_FORBIDDEN
```

- `reviews/**`의 과거 PR·적대적 검토 기록.
- `benchmarks/**`의 Evidence Pilot.
- `archive/**`.
- 완료된 PR·commit·CI run·Sheet 변경 이력.

`[증거]`는 과거 사실을 증명하지만 현재 규칙을 자동 변경하지 않는다.

## 6. 신규 작업자 규칙

1. `PROJECT_CORE.md`와 `DOCUMENTATION_MAP.md`를 먼저 읽는다.
2. 대상 파일이 `[현행]`인지 확인한다.
3. 건물 Tier 작업은 `APPROVED_OMENWARD_BUILDING_TIER_REALIGNMENT_2026-08-06.md`를 우선한다.
4. 특수병 병영 T1에 TokenSource를 추가하지 않는다.
5. 자동생산과 TokenSource 공급을 같은 사건으로 취급하지 않는다.
6. Stage 1에 특수병 병영을 의무화하지 않는다.
7. Stage 1에 마력탑을 설치해도 전술 연구 설명은 Stage 3까지 열지 않는다.
8. 세팅 확인 전 무료 재배치를 확인 뒤 본편 규칙으로 누출하지 않는다.
9. 필수 비용 예약을 우회하는 지출과 부분 건설 거래를 금지한다.
10. 나머지 `PENDING_GRILLME`를 임의 해소하지 않는다.
11. 정확 수치는 시뮬레이션·Codex 계획·제품 RED 테스트 뒤에만 확정한다.

## 7. 완료 이력

```text
OMW-DEC-20260805-PLANNING-STAGE-END-MERCHANT-V1
6_OF_10
OMW-DEC-20260805-PLANNING-TACTICAL-SKILLS-AND-MANA-V1
5_OF_10
OMW-DEC-20260805-PLANNING-TROOP-ROLES-SYNERGIES-AND-COUNTERS-V1
4_OF_10
OMW-DEC-20260805-PLANNING-SIX-BUILDING-T2-T3-BRANCHES-AND-COUNTERS-V1
3_OF_10 / SUPERSEDED_BY_BUILDING_TIER_REALIGNMENT
LEGACY_C1_C2_C3_PROVEN
```
