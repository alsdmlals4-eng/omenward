...
- 첫 세션은 실제 MapRun이다.
- 시스템은 실제 목표와 선택에 필요한 시점에 단계적으로 노출한다.
- 실제 경제·전투 결과 규칙을 사용한다.
- 벨루는 안내자이며 플레이어의 건설·룰렛·배치·구매 선택을 대신하지 않는다.
...
과거 마력탑 분기·구형 자원명·상시 상점·무한 재고·직접 핵심 보상 판매·별도 튜토리얼·Stage 1 전체 시스템 덤프·scripted victory는 `[대체됨] / IMPLEMENTATION_INPUT_FORBIDDEN`이다.
