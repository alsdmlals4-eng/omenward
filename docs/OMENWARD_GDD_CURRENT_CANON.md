...
Stage 1에는 플레이어의 직접 건설이 없다. Ward Citadel은 보이는 생산 건물·울타리 없이 고정탑 2개와 잠긴 패드 4개로, 각 Ward 전진기지는 열린 전장 안의 자동공격탑 1개와 잠긴 패드 2개로 읽힌다. 고정 전진 바리케이드는 없다. 자동공격탑은 전선 국소 화력만 지원하며 점령력이나 병력 커밋 대체 수단이 아니다. 안정적으로 점령한 거점의 별도 건설 노드는 Stage 2 이후에만 해금 건물을 설치할 수 있고, 점령 중에는 잠긴다. 정확한 구조는 `docs/design/APPROVED_OMENWARD_OPEN_BATTLEFIELD_TOWER_ONLY_FORWARD_LAYOUT_2026-08-28.md`가 소유하며 아직 runtime 구현이 아니다.
...
- `docs/design/APPROVED_OMENWARD_OPEN_BATTLEFIELD_TOWER_ONLY_FORWARD_LAYOUT_2026-08-28.md` — current 열린 본진/전진기지 node·tower count·경계 owner.
