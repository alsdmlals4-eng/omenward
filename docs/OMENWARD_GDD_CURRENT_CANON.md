...
현재 runtime package에는 Priest support, support fallback, flying priority/permission 분리, cluster deterministic tie-break, Giant collectors, registered deterministic FV fixtures, true per-cast `TARGETS_HIT_PER_CAST`의 7개 gap이 남아 있다. 이 planning/canon Decision에서는 persistent PowerShell/Codex/Godot 구현을 시작하지 않는다.
