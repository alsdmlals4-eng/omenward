...
current_status: PARTIAL_APPROVAL_8_OF_10 / DRAFT_PR / NOT_IMPLEMENTED
current_working_pr: 142
previous_completed_decision: OMW-DEC-20260805-PLANNING-STAGE-END-MERCHANT-V1
next_grillme: BELU_INTERVENTION_AND_FAILURE_RETRY_SKIP_RULES
...
첫 10~15분 흐름 7/10의 체크포인트 8까지 부분 정본화한다. Stage 1의 여섯 T1 직접 설치·건물 Tier 재정렬·Stage 2 방패병/궁병 선택과 실제 골드 규칙을 유지하면서, 특수병 병영 T1의 병영별 1회 무작위 선정·즉시 공개·저장 재추첨 방지 계약을 연결한다.
...
- 특수병 병영 T1은 건설 성공 시 병영별로 특수병 5종 중 하나를 한 번 추첨한다.
- 결과는 건설 확정 직후 공개하고 이름·아이콘·역할·첫 생산 카운트다운을 표시한다.
- 특수병 병영 T1은 선정 병종만 반복 자동생산하고 TokenSource는 제공하지 않는다.
- 저장·불러오기·체크포인트 복구로 T1 결과를 다시 뽑지 않는다.
- 결과 공개 뒤 무료 취소·전액 환급으로 무료 재추첨하는 경로를 금지한다.
- T2에서는 T1 결과와 무관하게 마도사·사제·암살자·비행병·거인 중 하나를 선택하고 TokenSource를 해금한다.
...
CHECKPOINT_7_BOUNDED_GREEN = 8_PASS / 0_FAIL / 0_ERROR
CHECKPOINT_8_RED_COMMIT = 42255ccc1601aeb7bc63f74b06373d8b14b5298f
CHECKPOINT_8_RED_RESULT = 1_FAILURE / 8_ERRORS_AS_EXPECTED
CHECKPOINT_8_BOUNDED_GREEN = 9_PASS / 0_FAIL / 0_ERROR
CHECKPOINT_8_PY_COMPILE = PASS
...
다음 Gate는 벨루의 온보딩 개입 수준과 실패·재시도·스킵 규칙이다.
