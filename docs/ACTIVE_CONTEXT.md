...
2026-09-20 P04 원격 readback: 구현 f0681f81ae61ddee5f7b8a5f53c4ea58ca11704c의 정확한 PR259 HEAD에서 원격6개 PASS, 로컬/origin 일치. 커밋 후 부모 대비 exact-path 범위 검사도 PASS. 아래 이번 로컬779/66/GPU는 이 구현에 해당한다. 기존 전체 검토 예산은 초기화하지 않았으며 CI adversarial-gate는 이번 독립 코드 검토의 증거가 아니다. 격리시험6폴더114파일498652bytes는 해시 검증 후 Downloads/OMENWARD_DELETE_REVIEW_20260912/grade-procs-20260920로 이동만 했다. 기존 월간 README에 누적, PDF 재발행 없음. 남은27 addon import 및 원 작업폴더 사용자 기획 변경은 보호했다. 부모258/PR259 Draft와 main90493888은 유지한다. 이후 문서 마감 커밋의 원격 결과는 PR Checks에서 별도 확인한다.

2026-09-20 P04 후속: 새 원정 proc_v1에 정예 궁병5타 관통·숙련 창병 저지 둔화·정예 창병6초 반격을 연결했다. BASIC/SECONDARY/COUNTER를 동기 분리해 추가 발동 재귀를 막고 사건 순번·횟수·재사용 시간을 저장한다. 구형 marker 없는 저장은 기존 규칙 유지. 최종 로컬 모델779/저장66/화면/기본씬 PASS, 효과 집중64 및 문서/범위 Python97 PASS. GPU 시험 편성에서 실제1tick 전투·반격6초 툴팁을 확인했다(output/front-status.png); 자연 정예 획득·사람 재미·최종 연출 검증 아님. 새 이미지/Aseprite 작업 없음. 기존 실행 계획 상단에 원문·범위·한계 기록. 독립 전체 검토2/2는 계승하되 이번 증분에 새 독립 검토를 했다고 주장하지 않는다. 다음은 P04 방패 인접보호·마법사/대검 잔여 효과부터, 이후 P05~P09. 원격 판정은 이번 커밋의 실제 PR259 Checks 확인 전까지 대기이며 Draft/main 경계 유지.
