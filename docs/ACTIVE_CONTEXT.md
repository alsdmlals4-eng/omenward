...
Historical C0 local closure owner remains:
...
CLEAN_ARCHIVE_NORMAL_HEADLESS_BOOT = CRASH_SIGNAL11
NEXT_EXECUTABLE_STEP = DISPOSABLE_AUTOLOAD_AB_ISOLATION
```

The clean exact-head archive reproduced the earliest crash without the active local `.godot` cache and without the user's uncommitted Issue #176 test deltas. This isolates the blocker to committed project startup state or below, but does not yet identify a specific autoload or main-scene component.
...
Historical session/PID values are evidence only and are intentionally not current selectors here. Fresh session/process identity is required before every local mutation block.
