...
current_activity: BATTLE_PRIMARY_MARCH_MINIMAP__IMPLEMENTED__FOCUSED_MACHINE_VERIFIED__RUNTIME_NOT_RUN
...
current_battle_focus_owner: scripts/ui/battle_focus_view.gd__IMPLEMENTED__FOCUSED_HEADLESS_CONTRACTS_PASS
...
implementation_execution: IMPLEMENTED__FOCUSED_HEADLESS_GODOT_CONTRACTS_PASS__RUNTIME_NOT_RUN
...
current_chat_runtime_status: BATTLE_PRIMARY_HEADLESS_CONTRACTS_PASS__LIVE_CAPTURE_NOT_RUN__HUMAN_NOT_RUN
...
RUN_COMMAND_IMPLEMENTATION_EXECUTION = BATTLE_PRIMARY_MARCH_MINIMAP_IMPLEMENTED__FOCUSED_HEADLESS_CONTRACTS_PASS__RUNTIME_NOT_RUN__HUMAN_REMAINS
HISTORICAL_PRE_APPROVAL_GATE = IMPLEMENTATION_AUTHORITY_REQUIRED
PROJECT_ACTIVITY = BATTLE_PRIMARY_MARCH_MINIMAP_IMPLEMENTED__MACHINE_VERIFIED__RUNTIME_AND_HUMAN_REVIEW_PENDING
...
GODOT_CODEX = BATTLE_PRIMARY_MARCH_MINIMAP_IMPLEMENTED__FOCUSED_HEADLESS_CONTRACTS_PASS__RUNTIME_NOT_RUN
...
`HISTORICAL_PRE_APPROVAL_GATE`는 2026-08-24 implementation 승인 이전의 과거 gate를 보존하는 호환 marker이며 현재 다음 작업이 아니다. 기존 승인 Shield Guard pair와 legacy backdrop은 새 전투 우선 화면의 headless contract에서만 확인했다. 새 단일 전선 지형은 `GENERATED_CANDIDATE`이며 아직 `assets/` 또는 Scene에 연결되지 않았다. 현재 gate는 battle-primary의 실제 기술 스모크와 사람 사용성 확인이다.
...
- `docs/qa/OMENWARD_SINGLE_MARCH_FRONT_RUNTIME_SMOKE_2026-08-30.md` — technical runtime smoke only; human usability remains open.
...
CURRENT_UI_EVIDENCE = MACHINE_CONTRACT_PASS__LIVE_CAPTURE_NOT_RUN
...
CURRENT_HERA_LIVE_QA = NOT_RUN__BATTLE_PRIMARY
