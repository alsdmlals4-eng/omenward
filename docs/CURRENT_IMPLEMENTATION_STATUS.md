...
latest_planning_status: PARTIAL_APPROVAL_8_OF_10 / DRAFT_PR / NOT_IMPLEMENTED
...
- Stage 2 방패병·궁병 후보 비교와 하나 업그레이드 처리.
- Stage 2 실제 골드 지급·후보 비용 예약·후보 외 소비 차단·기존 골드 보존.
- 선택한 일반병 병영의 자동생산·TokenSource 변경과 첫 Stage 2 룰렛 전후 비교.
- 미선택 분기의 후속 접근 안내와 같은 병영 분기 변경 금지.
- 두 경로가 모두 유효한 혼합 지상 압력 검증.
- 특수병 병영 T1의 건설 성공 후 병영별 1회 무작위 선정.
- 선정 결과 즉시 공개와 첫 생산 카운트다운.
- T1 선정 병종 고정 반복 생산과 저장·복구 시 결과 보존.
- 무료 취소·전액 환급·저장 불러오기를 통한 재추첨 방지.
- T2 전문화 시 플레이어 선택 병종으로 교체하고 TokenSource를 해금하는 처리.
...
위 구조의 자동생산, TokenSource, 업그레이드 데이터, 건물 UI와 전장 동작은 제품에 반영되지 않았다. 생산 간격·비용·토큰 가중치와 방어탑 T3는 미정이다.

## Stage 2 최초 T2 후보·골드 규칙 — 승인·미구현
...
방패병·궁병 비교 UI, 실제 골드 예약 거래, 병영 Tier 변경, 자동생산·TokenSource 변경, 첫 Stage 2 룰렛 비교, 혼합 압력 전투는 제품에 구현되지 않았다. 같은 비용에서 두 경로의 총가치와 룰렛 변화 체감은 시뮬레이션·사람 QA 전 미검증이다.

## 특수병 병영 T1 선정·공개 규칙 — 승인·미구현
...
건설 거래·RNG 결과 저장·결과 공개 UI·생산 타이머·저장 복구·재추첨 방지·T2 전환은 제품에 구현되지 않았다. 다섯 병종의 가중치·생산시간·비용·환급률과 모든 결과의 유효성은 시뮬레이션·사람 QA 전 미검증이다.
