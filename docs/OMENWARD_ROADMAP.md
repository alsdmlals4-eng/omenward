...
- 건설 전에 룰렛 확률 변화와 비용·효과를 읽는다.
- 전투 중 상성·사거리·현재 타깃을 읽고 웨이브 뒤 실제 성공·실패 원인을 확인한다.
...
- 실제 화면 크기에서 병종·진영·등급·공격 전조와 C3 HUD를 읽을 수 있다.
...
| C0 정본·코어 복구 | 프로젝트 코어·상태·로드맵 일치 | **완료 / CORE_LOCKED** | 정본 유지 |
...
이 표와 1절이 현재 상태의 권위 원본이다. 아래 상세 절은 완료 기준·불변 조건·검증 계약의 보존 기록이며 과거 상태 문구가 현재 위치를 덮지 않는다.
...
**완료.** 보존 기록이며 현재 작업 입력으로 직접 재사용하지 않는다.
...
- 과거 Phase 0 계획 게이트와 목표.
- 당시 Git 이력의 작업 입력.

### Codex 조사·제출 항목

- 당시 main의 실제 파일·경로·테스트·실행 명령.
- Godot 버전·renderer·화면 정책.
- Base의 작업 방법과 검수 기준.
- 실제 Scene 트리와 상태 소유.
- UnitArchetype·Tier·Rank·FactionVisual·AnimationContract 구조.
- Battlefield·Manifest·시간·RNG·로그 경계.
- 단계별 구현·자동/수동 검증·위험·사용자 결정 요청.

### 금지

- 승인 전 `project.godot`, Scene, GDScript, Resource, 데이터, 테스트 수정.
- 승인 전 구현 브랜치·커밋·PR.
...
- Visual Contract probe와 headless 테스트 러너.
- README 실행·검증 명령.
...
- Godot editor/headless 로드와 test runner PASS.
...
- 좌우 대칭 독립 3라인 그레이박스와 성문 6개.
- 중간거점·접전지·전방 2·후방 1 노드.
- 최소 건물 2종과 3×3 룰렛·결과 보관·배치.
- 공용 아키타입 대표 병종.
...
현재 상태: 과거 계획 게이트 완료. 실제 상태는 C1·C2·C3 절을 따른다.
...
- 1080p·720p에서 확률·토큰·징조·사거리/대상·원인·건설 비교가 읽힌다.
- 목표 객체 수에서 성능 기준을 측정한다.

현재 판정: 구성요소와 C3 자동 계약은 존재하지만 **사람 플레이 미실행**.
...
- C1U 결정 뒤 룰렛 최소 100,000시드 시뮬레이션.
- 금화 EV와 럭키 확률.
...
Work Order, Codex 제안서, 승인된 구현 결과, 자동 검증, 사람 검증을 명확히 구분한다.
