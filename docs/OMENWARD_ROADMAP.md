...
2026-09-20 P04 증분 완료: 현재 동기 공격의 비재귀/재처리 방지·저장 재개와 궁병 관통/창병 저지·반격을 연결했다(모델779/저장66/화면/기본씬, GPU 효과 시험 편성). 비동기 투사체 queue는 실제 소비처와 함께 P07에서 확장하며 현재 구현으로 완료 선언하지 않는다. 다음은 방패 인접 아군보호 → 마법사4타 범위·대검 정예 방어감소 → 기병/거인/암살자/비행 잔여 효과와 T3 교차 회귀 → P05 영웅. 사람의 선택/재미·자연 정예 성장·후속 맵/최종아트·main 통합은 별도 남는다. 자세한 구현/검증/실패 이력은 기존 visible-battle 계획 상단 소유.

2026-09-20 후속: 첫 맵4정책×3seed/실제 저장 재개 비교를 추가하고 발견한 float 저장 정밀도 결함을 우선 교정한다. 정책별 승패와 한계는 기존 visible-battle 계획 상단 소유. 첫 맵 반복·다른 정책의 자동 근거는 추가됐지만 사람 선택/재미·후속 맵 균형은 여전히 미검증이다. P04 사건의 비재귀·저장 계약 → 잔여 등급 효과 → P05 이후 순서는 유지하며, 이번 비교로 규칙·수치를 임의 재승인하지 않는다.

2026-09-20 통합 준비와 첫 맵 정보/조작 검토를 기존 계획에 누적했다. 다음 공세 배정 표시, 동일 시뮬레이션의 확대/복귀, 실자원 첫 맵 자동 정책의 종결까지 연결됐다. 사람 재미·다른 조작 정책·후속 맵 균형은 미검증이다. 다음 순서: 첫 맵의 증원 선택/원인 이해 관찰 및 필요한 한정 교정 → P04 잔여 등급/비재귀 사건·저장 계약 → P05 영웅 → P06~P09. 이번 묶음은 기획/게임 Draft 전체 병합이나 출시 승인이 아니다.

2026-09-16 사용자 우선 요청: 3전선 복귀, 기본 미니맵과 선택 전선 확대를 구현하고 기존 월간 일지에 당일 요약을 누적한 뒤 작업 브랜치를 동기화한다. 현재 검증/통합 결과는 Active Context를 따른다. 다음 기능 작업은 이 구조에서 P04 잔여 등급·비재귀 사건/저장 계약 → P05 영웅 → P06~P09 UI·아트/모션·전체 원정 검증 순서이며, 이번 요청을 전체 게임 완료로 표시하지 않는다.

2026-09-14 P04 선행 범위선택 교정 완료(435모델/55저장/UI·기본실행): 전방 대검, 거리→ID 제한 타격, 구형 숫자ID 타이브레이크. 다음 등급 효과 구현 시 이 공통 선택 경계를 재사용하고 secondary/counter 비재귀·중복 방지·저장 재개 계약부터 연결한다. 새 GPU/Human/main 증거는 없음.

2026-09-14 T3 구매/생산/징조/10capstone 연결 및426모델·55저장·화면·GPU fixture 검증. 다음 실행은 P04의 나머지 숙련·정예 효과와 재귀 없는 추가타/반격, 그 뒤 P05 영웅. P03 T3 미연결 문장은 이전 시점 이력. 전체원정 밸런스·최종모션·Human/main 통합은 아직 미완료.

2026-09-14 P03 출생정보 연결: 시설 instance/병력 entry/tier/source, 징조 토큰·최저티어 보너스, 확정 전 티어 표시와 정확한 병력 출전, 신규 marker/구형 저장 보호를 연결했다. 다음은 T3 동일 병종 심화의 비용·6라운드 gate·이후 생산/징조 토큰과 실제 capstone 효과다. 검사는 Active Context의 최신 readback을 따른다. 전체 P04~P09/자연5맵/Human/main은 남는다.

2026-09-14 P03 T2 맵간 해금 유지 및 공통 UI 판정 연결: 모델368/저장51/화면/기본실행 PASS. 다음은 출생 tier/source/대기 entry의 저장·생산·징조확정·출전 소비 계약을 먼저 연결하고 T3 심화를 적용한다. 시설 슬롯 재사용 시 출처 혼동 및 기존 병력 소급 강화 금지를 회귀로 검증한다. 이후 P04~P09 및 자연5맵/Human/main 경계 유지.

2026-09-14 P03 고정슬롯·철거·재건설·희소저장·확인UI 연결 및366모델/51저장/화면/기본실행/GPU 확인창 검증. P03 나머지: T2 맵간 gate, T3 전문화, 생산/징조확정의 출생tier·source·대기entry와 실제 전투 소비처. 기존 아트는 보존했고 새 그림 생성은 없었다. 이후 P04~P09 순서 유지; 자연5맵완주/최종Human/main 미완료.

2026-09-14 최신: P03 군수소 수용량/목록UI/비활성/저장 보호 증분349모델·51저장·화면·기본실행 검증. 다음 P03 고정슬롯/철거/T3/생성 병력 출생정보와 후속 P04~P09를 계속한다. 사용자 추가의 월간 증빙집은 구현 원본을 재사용한 별도 파생보고서로 우선 준비한다. 향후 분리 아트는 크로마키 후 배경제거·alpha/모션/consumer 검증. 새 이미지 생성·최종아트·자연5맵완주·Human/main 통합을 이번 완료로 간주하지 않는다.

P02 진행: 시간제 거점/본진 claim/기본보급정산/UI 소비처 및 GPU 전투 연결. 최종 타워 사망 경계 검증 후 P03 고정슬롯·군수·T3출생으로 이동한다. 투사체·탑 전용아트·전체자연완주는 아직 남는다. 이전 P01 event skeleton은 실제 추가타/투사체 consumer와 함께 구현하며 미사용 빈framework를 만들지 않는다.

2026-09-14 연속 구현: P01 고정30Hz/저장 호환/UI2× 증분 검증 진행. 이어서 P01 남은 사건·식별 경계 → P02 점령/정산 → P03 군수/T3 → P04 병종 → P05 영웅 → P06 전체화면 → P07 투명아트/모션 → P08 완주/밸런스 → P09 전달 순서로 진행한다. 아래 계획-only와 이전 next는 당시 이력이다.

2026-09-14 승인 후 실행: P00 안정 진입점/현재owner routing 갱신, P01 저장 transport·검증된 backup 복구 증분 연결 및 모델268/저장43/화면·scope7 검증. P01 전체 완료는 아님. 다음은 기존 원정 호환을 유지하는 고정30Hz 시계·정수 duration/v7 경계→P02 점령·정산이며 아래 P00–P09 순서는 유지한다. 새 이미지/게임 수치 변경 없음.

2026-09-14 현재 실행 순서: [남은 작업 설계·구현 명세](superpowers/plans/2026-09-11-visible-battle-slice.md)의 P00 정본/범위 → P01 고정tick·저장 → P02 점령·정산 → P03 군수·T3출생 → P04 등급/전투 → P05 영웅 → P06 전체UI → P07 투명아트/모션/5맵 → P08 공세·밸런스·오디오 → P09 통합·전달. 이 순서가 아래의 과거 next를 대체한다. 이번은 계획만 준비하며 구현 시작/검증 완료를 기록하지 않는다. 모든 패킷에 실제 변경 경로, 경계, 저장 호환성, 합격 반례를 연결했다.
