...
2026-09-10 우선 순서: 핵심 유지·연구 비교 → 화면/흐름 Blueprint → 방패병 V3 품질 방향 후속 진행 → 비인간 베일 pair·화풍 적합성 시안 제작(현재 검토 대상) → 투명 경계·실제 배치 → 대표 병종 모션/Aseprite pilot → Godot 검증 → 콘텐츠 확장. V2는 배치 참고이며 캐릭터 품질 기준에서 제외한다. 합성 시안은 실제 엔진 배치 증거가 아니다. 현재 상세 owner는 `docs/design/OMENWARD_COMMAND_FLOW_AND_MOTION_BLUEPRINT_2026-09-10.md`다.
