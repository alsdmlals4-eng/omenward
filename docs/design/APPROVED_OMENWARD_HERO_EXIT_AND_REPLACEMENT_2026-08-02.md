...
전선에 배치한 이름 지정 `[영웅]` 유닛은 플레이어가 수동으로 퇴각시키거나 다른 영웅으로 교체할 수 없다. 영웅은 배치한 전선에 비가역 커밋되며, 살아 있는 동안 Stage와 Act가 전환되거나 Stage 사이 정비시간에 진입해도 같은 전장 유닛 인스턴스로 계속 출전한다.
...
- Stage 사이 정비시간에도 영웅을 임의로 귀환시킬 수 없다.
...
- Stage 정산 뒤 `MaintenancePhase` 진입도 active hero 슬롯을 비우지 않는다.
...
- 체력·쿨다운·일시 상태·소환물의 정확한 Stage 정산·정비시간·다음 Stage 처리는 다음 Decision에서 확정한다.
