...
status: CURRENT_TIER_LINEAGE_AUTHORITY / STAGE1_ENTRY_RULE_PARTIALLY_SUPERSEDED / NOT_IMPLEMENTED
...
## 2A. 2026-08-28 Stage 1 범위 정정

`OMW-PLAN-20260828-STAGE1-PREBUILT-EXPLAIN-01` 및 `OMW-PLAN-20260828-OPEN-BATTLEFIELD-TOWER-ONLY-01`이 이 문서의 **Stage 1 직접 설치와 고정 전진 바리케이드** 문구만 대체한다. 이 문서의 건물 Tier·전문화 계보는 유지한다.

```text
STAGE_1_SIX_T1_DIRECT_INSTALL = SUPERSEDED_FOR_STAGE_1_ONLY
STAGE_1_DIRECT_CONSTRUCTION = FORBIDDEN
STAGE_1_VISIBLE_PRODUCTION_BUILDINGS = NONE
STAGE_1_HOME_COMMAND_ROOT = 4_LOCKED_CONSTRUCTION_NODES + 2_FIXED_AUTO_ATTACK_TOWERS
STAGE_1_FORWARD_BASE_DEFENSE_SYSTEM = 2_LOCKED_CONSTRUCTION_NODES + AUTO_ATTACK_TOWER_ONLY
FORWARD_BARRICADE = REMOVED__NOT_A_FIXED_DEFENSE_OR_MAP_VISUAL
FIRST_MEANINGFUL_BUILD_OR_UPGRADE = STAGE_2_T2_UPGRADE
OCCUPATION_NODE_BUILDING_CHOICE = STABLE_PLAYER_OWNED_OUTPOST_ONLY
```

따라서 “Stage 1에서 반드시 설치하는 6종 T1 세트”는 현재 FTUE 명령이 아니라 historical planning input이다. Stage 2의 첫 T2 선택과 이후 안정 점령 거점 노드에서의 건물 선택은 이 문서의 Tier 계보를 참조하되, 실제 roster·비용·node 제약은 별도 구현/시뮬레이션 계약으로 확정한다.
