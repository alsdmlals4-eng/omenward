...
- 세계관·명칭.
- 벨루 단일 안내자.
- 좌우 대칭 독립 3라인, 라인별 성문, 점령 가능한 중간거점, 독립 중앙 접전지.
- 중간거점 전방 2·후방 1 건설 노드와 점령 시 건설권·생산권 이전.
- 암살자는 직접 후방 배치하지 않고 평상시 안개로 가려진 같은 라인 우회로를 사용.
- 전장 전체 조망을 기본으로 하며 별도 미니맵은 사용하지 않음.
...
- 전장 연결 구조, 라인별 성문, 중간거점 점령, 노드 배치, 우회로 표시 방식 변경은 사용자 승인 대상이다.
...
- 중간거점 생산·점령·건물 처리 수치 확정.
- 암살자 우회 시간·탐지·도착 범위 확정.
...
이후 작업은 새로운 구조를 무분별하게 추가하기보다 이 기준의 수치·이름·아트 품질을 검증하고 다듬는 단계다.
