...
- 상태: **11단계 구조 초안 완료 / 전장 3묶음·병종 애니메이션 연출 기준 승인 / 수직 슬라이스 Plan Mode 준비 단계 / 구현 미승인**
- 작성일: 2026-07-16
- 최신 갱신일: 2026-07-16

사용자 결정에 따라 세부 밸런스와 프레임 수는 첫 PoC 가설로 두되, 구현이 뒤집히지 않도록 역할·상태·데이터·검증 경계를 먼저 고정한다.
...
10. HUD·아트·오디오·병종 애니메이션·전투 연출 제작 계약.
...
- 병종은 무기·무게중심·이동 리듬·공격 준비·충돌 키포즈로 구분한다.
- 공격 판정과 무기 접촉·투사체 발사 프레임은 한 프레임 이내로 맞춘다.
- 스테이지 승리 모션은 병종별로 제공하되 전투 중 반복 축하 연출은 사용하지 않는다.
...
- 필수 상태: 배치, 대기, 이동, 기본 공격, 핵심 스킬, 가벼운 피격, 사망, 승리.
- 역할에 따라 강한 피격, 제어, 점령, 패배, 지속 시전을 추가한다.
- 암살자는 우회 진입·출현, 기병은 돌진·방향 회복, 비행병은 상승·순항·급강하, 거인은 구조물 공격을 별도 상태로 가진다.
- 일반 인간형 첫 프레임 가설: 대기 4~6, 이동 6~8, 기본 공격 6~10, 핵심 스킬 8~14, 사망 6~10, 승리 8~14.
- 공격은 준비→판정→회복 구간을 분리한다.
- 가벼운 연속 피격은 큰 경직 대신 짧은 플래시·미세 반동으로 처리한다.
- 웨이브 정리 시 완전한 승리 모션을 사용하지 않고, 스테이지 승리 확정 뒤 2.5~4초 병종별 승리 시퀀스를 사용한다.
- 카메라·화면 흔들림·히트 스톱은 다른 라인의 판독을 방해하지 않는 범위로 제한한다.
...
- 애니메이션 재생 FPS, 프레임 수, 공격 이벤트 프레임, 히트 스톱·카메라 흔들림 수치.

## 변경 규칙

- 병종 삭제·통합·건물 계열 변경은 사용자 승인 대상이다.
- 전장 연결 구조, 성문 수, 중간거점 노드 배치, 점령권 이전, 우회로 표시 방식 변경은 사용자 승인 대상이다.
- 병종별 이동·공격 정체성, 판정 동기화, 승리 연출 단계 변경은 사용자 승인 대상이다.
- 초기 수치와 프레임 수는 플레이테스트 근거가 있으면 같은 구조 안에서 변경할 수 있다.
...
- 대표 체형의 대기·이동·공격 키포즈와 40px 축소 모션 테스트.
- 승인된 아트 규격에 따른 실제 스프라이트·아이콘 제작.
- 최종 팔레트·캔버스 크기·방향 수·애니메이션 FPS 결정.
