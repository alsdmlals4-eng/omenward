...
approval: USER_APPROVED_RECOMMENDED_OPTION_WITH_NO_RECOVERY_REWARD_CLARIFICATION
status: USER_APPROVED_FRESH_INSTANCE / NO_DEATH_RECOVERY_REWARD / NEW_ROULETTE_RESULT_REQUIRED / NOT_IMPLEMENTED
...
같은 영웅이나 다른 영웅을 다시 출전시키려면, active 영웅 슬롯이 빈 상태에서 룰렛을 정상적으로 다시 조작해 해당 병종의 새로운 `[영웅]` 등급 토큰을 획득해야 한다. 새 토큰을 소비해 생성한 영웅은 이전 사망 인스턴스와 분리된 완전한 새 유닛 인스턴스로 시작한다.
...
AND roulette final result creates a new Hero-grade token
AND token.UnitArchetype matches selected unlocked Hero
→ Hero conversion and deployment may be offered
```

- 영웅 사망만으로 새 토큰을 생성하지 않는다.
- 이전 출전의 source token을 복구하거나 복제하지 않는다.
- 보관함에 이미 존재하던 새 미소비 `[영웅]` 등급 토큰이 있다면 그것을 사용할 수 있다.
- 새 토큰이 없다면 룰렛에서 `[영웅]` 등급 결과가 다시 나올 때까지 이름 지정 영웅을 배치할 수 없다.
- 룰렛 결과가 영웅 등급이어도 병종이 일치하지 않으면 해당 영웅의 재출전에 사용할 수 없다.
- active 슬롯이 비어 있어도 일반·다른 등급 토큰을 영웅으로 승격할 수 없다.
- 영웅 사망을 이유로 과거 `SpinSnapshot`, 현재 보상, 다른 토큰 또는 릴 구조를 변경하지 않는다.

## 4. 새 인스턴스의 초기 상태

새 `[영웅]` 등급 토큰을 소비해 생성한 이름 지정 영웅 인스턴스는 다음 상태로 시작한다.
...
active 슬롯이 비고 새 토큰이 준비되면 다음 선택이 가능하다.
...
- 새 인스턴스 생성은 실제 새 토큰을 한 번 소비해야 한다.
...
- 토큰 소비와 새 유닛·배치 기록 생성은 원자적으로 처리한다.

## 7. UX 책임

- 영웅 사망 시 `영웅 슬롯 해제`와 `토큰 반환 없음`을 명확히 표시한다.
- 사망 화면에 회수·부활·재배치 보상처럼 보이는 연출을 사용하지 않는다.
- 새 영웅 출전 조건에 `새 동병종 [영웅] 등급 토큰 필요`를 표시한다.
- 새 토큰이 없으면 영웅 후보 버튼을 비활성화하고 필요한 조건을 설명한다.
...
| 사망 즉시 같은 영웅이 자동 생성된다 | 유효 | 정상 룰렛의 새 동병종 영웅 등급 토큰을 필수 조건으로 둠 |
| 일반 토큰을 영웅으로 승격해 희귀도 비용을 우회한다 | 유효 | 실제 `[영웅]` 등급 토큰만 변환 가능 |
| 이전 사망 인스턴스의 낮은 HP를 버리고 새 토큰으로 완전 회복한다 | 의도된 새 토큰 가치 | 별도의 희귀 토큰을 다시 소비하는 완전한 새 인스턴스이며 무료 부활이 아님 |
| 같은 토큰을 저장·재시도로 두 번 소비한다 | 유효 | 토큰 소비·유닛 생성·배치 기록을 원자 처리하고 fault test 필요 |
...
NEW_MATCHING_HERO_GRADE_TOKEN: REQUIRED
NEW_ROULETTE_RESULT_IF_NO_STORED_TOKEN: REQUIRED
