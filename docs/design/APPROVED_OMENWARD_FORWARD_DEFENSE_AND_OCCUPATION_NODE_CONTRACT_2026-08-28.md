...
- 본진과 전진기지는 양 진영에 같은 구조 언어를 사용한다.
...
Stage 1은 직접 건설을 요구하지 않는다. 첫 화면에서 네 개의 건설 메뉴를 차례로 열지 않고, 다음 **세 가지 설명 단위**로 이미 지어진 시설을 읽게 한다.

```text
1. 일반병 병영: 자동생산과 룰렛 TokenSource의 분리
2. 농장: 병력 한도와 장기 유지
3. 전진기지 방어 체계: 바리케이드 + 자동공격탑
4. 3×3 룰렛: 결과를 제한적으로 수정
5. 한 전선 비가역 커밋: 고정 방어가 아닌 플레이어 결정이 결과를 만듦
```
...
TEST_STAGE1_EXPLAINS_THREE_FACILITY_UNITS_BEFORE_ROULETTE_AND_REQUIRES_NO_DIRECT_BUILD
```
