...
status: USER_APPROVED_SINGLE_ACTIVE_LIMIT / EXIT_AND_STAGE_STATE_RULES_APPROVED / NOT_IMPLEMENTED
...
- 사망 뒤 새 토큰으로 생성되는 새 인스턴스의 초기 HP·쿨다운·충전·고유 자원은 별도 다음 Decision이다.
...
퇴각·교대·종료 사건의 주 책임 원본은 `APPROVED_OMENWARD_HERO_EXIT_AND_REPLACEMENT_2026-08-02.md`다. Stage 상태 지속의 주 책임 원본은 `APPROVED_OMENWARD_HERO_STAGE_STATE_PERSISTENCE_2026-08-02.md`다.
...
| Stage마다 HP·쿨다운을 초기화해 장기 비용을 우회한다 | 유효 | 후속 승인에 따라 장기 상태 유지·정비 중 clock 정지 |
...
= 사망 뒤 새 영웅 등급 토큰으로 생성되는 새 영웅 인스턴스의 HP·쿨다운·충전·고유 자원 초기값
```
