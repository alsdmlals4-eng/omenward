...
- 기본적으로 고유 자원·궁극기·새 AI·전체 신규 리그·전체 신규 애니메이션은 만들지 않는다.
- 외형은 스킨에 가까운 제작량을 목표로 하지만 단일 차이는 실제 전술 선택에 영향을 줘야 한다.
- 단일 차이의 전투 가치는 스탯·안정성·범용성에서 상쇄해 원본 병종 선택 상황을 유지한다.
...
- 기존 자동 발동 문서의 다중 능력 우선순위는 일반 확장 계약으로만 남고, 현재 이름 지정 영웅의 영웅 전용 차이는 최대 하나다.
- `스킨형`은 장식 전용을 뜻하지 않는다. 전투 선택을 바꾸는 차이 하나는 반드시 존재한다.
- `스킨형`은 무료 능력 추가를 뜻하지 않는다. 전투 예산 상쇄와 원본 선택 사유가 필요하다.
