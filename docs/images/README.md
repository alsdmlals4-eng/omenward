...
사용자 명시 정책: 사용자가 승인한 생성 이미지는 다음 두 보관처가 모두 확인되기 전에는 승인 등록을 완료로 처리하지 않는다.

1. 프로젝트 로컬 원본: `.asset-vault/library/...`의 변경하지 않은 PNG와 checksum 기록.
2. Notion 승인 자산 기록: asset ID, 승인 상태, 원본 접근 링크, 파일 메타데이터, 사용·제외 범위를 포함한 사람용 기록.

Google Drive 원본 링크는 장기 접근을 돕는 보조 보관처이며, 위의 프로젝트 로컬 파일 또는 Notion 기록을 대체하지 않는다. Godot `assets/`로의 import와 `IMPLEMENTATION_READY` 승격은 별도 승인·cleanup·runtime 검증 Gate를 따른다.
