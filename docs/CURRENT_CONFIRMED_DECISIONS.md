...
2026-09-20 기존 P04 구현 세부: 신규 원정만 combat_rules=proc_v1로 궁병 정예5타 관통(뒤1체·원피해50%)과 창병 숙련 저지 둔화30%/1초·정예 반격6초를 적용한다. 기존 marker 없는 저장은 legacy로 유지하며 자동 승격하지 않는다. 추가타/반격은 기본 적중횟수·집중사격·추가발동을 다시 실행하지 않고 저장된 사건 순번으로 재처리를 거부한다. 실제 brace 방어 후 살아 있고 기존 경직이 없는 창병은 그 공격의 후속 경직 적용 전에 반격한다. 이는 Blueprint 수치의 실제 소비 연결이며 새 밸런스/최종아트/Human 승인이 아니다. 현재 근거는 기존 visible-battle 계획의 P04 연속 실행 절을 따른다.

> 2026-09-20 우선 결정: 사용자가 승인한 이번 묶음은 [기획 통합 검사 교정과 3전선 첫 맵 검토](design/OMENWARD_REPLAN_AND_MOTION_INTAKE_2026-09-10.md#2026-09-20-승인된-통합-준비와-첫-맵-검토)다. 최신 사용자 3라인 복귀를 따른다: 전체 미니맵을 기본으로 하고 한 전선을 선택하면 캐릭터가 보이는 확대 전투 화면으로 전환한다. 후속 BUILD PR259의 승인·구현은 이 planning 브랜치/main과 구별한다. 아래 9/10 단일전선과 9/11 전체 구현 보류 문구는 당시 이력이며 현재 승인된 BUILD를 막는 권한이 아니다. 게임 규칙·저장·자산·다른 PR은 이번 운영 교정으로 바꾸지 않는다.
...
2026-09-16 사용자 확정: 신규 원정은 다시 3전선이며 기본 전황 미니맵 → 선택 전선 확대 → 전체 복귀 흐름이다. 5개 순차 맵/시간제 라운드/웨이브/재정비/두 승리조건은 유지한다. 권장 구현 세부는 공유 시계·경제·양측 본진, 전선마다 중앙 거점/방어탑 1개, 전체 적 공세량을 세 전선에 분배, 초기 병력 2/1/1이다. 선택은 이후 출전 목적지만 바꾸고 이미 출전한 병력은 옮기지 않는다. 기존 저장은 single_v1 그대로 유지한다. 기존 Blueprint의 단일 전선 문장은 신규 원정에 대해 superseded. 상세는 Blueprint의 2026-09-16 절과 현재 실행 계획을 따른다. 월간 작업일지는 기존 미제출 PDF에 날짜별 누적하며 별도 판을 매번 만들지 않는다. GitHub 동기화는 현재 작업 PR 범위이며 부모/main 우회 통합 승인이 아니다.

2026-09-14 P04 기존 범위공격의 구현 누락을 교정했다. 주표적은 한 번 보존하고 추가타 대상만 거리→숫자ID로 제한하며 대검 후방 추가피해를 제외한다. 기존 반경·대상수·등급·티어 수치는 변경하지 않는다. Godot 정렬의 비안정성을 고려한 명시적 타이브레이크이며 전투 배열 자체는 재정렬하지 않는다. 새 스킬 또는 전체 P04 완료 승인이 아니다.

2026-09-14 기존 P03/P04 명세의 T3 심화를 신규 birth_v2에 구현했다. 비용/해금/10병종 효과 수치는 Blueprint facility_progression/capstone_rules를 소비하며 이전병력 소급승급 금지. 정예 등급은 별개이며 정예 대검 등 잔여 trigger까지 이번에 완료한 것은 아니다. 타워는 원거리 피해로 분류해 암살자 원거리 방어에 포함한다. 자동/GPU 검증은 사용자 최종밸런스·Human·main 승인과 구분한다.

2026-09-14 P03 出生情報 구현 세부: 신규 birth_v1 원정은 시설 고유번호와 병력 출생 티어/출처/고유번호를 생산·징조확정·출전·저장에 보존한다. 같은 슬롯 재건설은 새 시설번호, 구형 문자열 병력은 T1/기본출처로만 취급한다. 혼합 징조 토큰의 기본3개 묶음·선택 완성선 보너스는 최저 티어를 적용한다. UI는 병종별 선입순 출전과 다음 병력 티어를 명시한다. 기존 위임의 권장 세부 구현이며 T3/최종Human 승인을 뜻하지 않는다.

2026-09-14 승인계획 P03 T2 gate 교정: 신규 원정의 전문화 해금은 첫 맵에서만 라운드 조건을 검사하며 다음 맵에서는 유지한다. 이는 기존 P03 명세와 구현의 차이를 교정한 것이며 새 사용자 최종 밸런스 승인으로 기록하지 않는다. 구형 저장의 gate는 소급 변경하지 않는다.

2026-09-14 승인계획 P03 연속 적용: 신규원정 slots_v1 시설 철거는 준비/재정비·징조미확정없음·활성시설에서만 무환급으로 수행. 사용자 게임내 확인 후 해당 슬롯만 비우며 기존 병력/확정대기는 유지하고 첫 해금 빈칸부터 재건설한다. 구형 원정에 새 철거 기능/희소배열을 소급 적용하지 않는다. 이 기능의 자동검사와 확인창 실행 증거는 최종 게임/Human 승인과 별개다.

2026-09-14 최신 사용자 운영 결정: 신규 분리 이미지 생성은 크로마키 배경 후 제거하며 기존 승인/알파/runtime 증거 경계를 유지한다. 블루프린트와 별도의 프로젝트명 월간 AI 활용 작업일지·증빙집 PDF를 지정 증빙 폴더에 만든다. 이 보고서는 Git/작업/검사 원본으로부터 파생되며 기획 정본을 복제하지 않는다. 실제 작업일과 사후 기록·캡처·발행일을 혼동하지 않고 확인되지 않은 계정·비용·협약을 입증된 것으로 쓰지 않는다. 세부 운영은 AGENTS의2026-09-14 추가를 따른다. P03 군수소35G/활성당+6은 전체 구현 위임 아래의 권장 세부값으로 적용했고 최종 밸런스/Human 승인은 아니다.

2026-09-14 P02 에이전트 권장 세부 적용: 새 원정은 지상 시간제 거점/본진 점령 및 잔여 기본보급1회 정산, 빈 거점 자동 확보 후 진군(후열 추격 암살자 예외)을 사용한다. 이는 전체 구현 위임 아래의 검증용 세부값이며 최종 밸런스/Human 승인 아님. 기존 저장은 기존 규칙을 유지한다. 정확한 수치 owner는 Blueprint JSON, 실행 차이와 증거는 현재 P00–P09 계획이다.

2026-09-14 최신 사용자 명시: 남은 명세를 조사·보완한 뒤 그 순서대로 게임 전체 구현까지 연속 진행하며 일상 승인/국소 완료에서 멈추지 않는다. 기존 계획의 P00–P09가 현재 실행 순서다. P01 고정 시계는 신규 원정만 적용하고 구형 저장 규칙은 보존한다. 새 최종아트·사용자 플레이·출시 승인과 보호된 main 통합은 기계 검사로 대체하지 않는다.

2026-09-14 후속 사용자 승인: ‘좋아 권장안대로 작업진행해’. 직전 P00–P09 계획의 구현을 재개한다. 아래 계획 전용 제한은 그 문서 준비 턴의 이력이다. 첫 변경은 P00 owner routing과 P01 저장 입출력 검증/이전 정상본 복구이며 기존 v1–v6 전투 규칙·Base lock·최종아트·main 보호는 유지한다. 고정 시계/v7은 저장 transport와 분리된 다음 증분이며 아직 구현 완료로 표시하지 않는다.

2026-09-14 최신 사용자 요청: ‘남은 작업과 해당 작업들의 구현,설계 명세를 준비해줘’. 이번 산출물은 [기존 실행 계획의 현재 잔여 명세](superpowers/plans/2026-09-11-visible-battle-slice.md)이며 게임 구현/새 이미지 제작을 이번 턴에 실행하지 않는다. 기존 확정 방향은 유지하고 모호한 점령 단계/출생 티어/이벤트 경계의 세부값은 RECOMMENDED_DEFAULT로 분리했다. 문서 검토가 새 수치·영웅·최종아트의 사용자 승인을 뜻하지 않는다. 아래 전체 구현 위임은 역사상 유효한 후속 목표이며 이번 요청의 계획 전용 범위를 덮지 않는다.

2026-09-13 최신 사용자 명시: ‘게임 전체 완성,구현까지 계속진행’. 국소 검토판 완료를 종료점으로 사용하지 않고 전체 기획 구현으로 이어간다. 기존 계획/코드의 first-map-only 중단 범위를 순차적으로 확장한다. 핵심 기획은 현재 Blueprint 기준, 새 최종 아트 승인·배포·Git 보호 우회는 포함하지 않는다.

2026-09-13 계속 위임 적용: 반복 승인 없이 기존 기획의 병종 대응과 시험용 첫 맵 조정을 구현한다. pressure0.4는 에이전트 권장 플레이테스트 값이지 사용자 최종 밸런스 확정이 아니다. 기존 저장 압력1.0 보존, 최종 아트/제품 승인 및 Git 보호 경계 유지. 세부 규칙/근거는 현재 Blueprint 입력과 실행 계획이 소유한다.

2026-09-13 최신 사용자 교정: 개선 루프는 유사 장르를 참고해 기획 구체화→시스템 연결→구현·실행 검증으로 게임 전체를 완성하는 반복이다. 국소 UI 수정만으로 축소하지 않는다. 매 단위 구현 전 계획을 먼저 수립한다. 이번은 기존 유료 치료 공식 구현과 실제 자원 운영 비교이며 결과를 다음 우선순위에 반영한다. 기술 선택은 반복 승인 없이 진행하되 핵심 의미 변경·최종 아트 승인·Git 보호 경계는 유지한다.

2026-09-13 사용자: 앞으로 구현/수정 계획을 먼저 수립한 뒤 진행. 직전 전투 가독성 권장 계획을 승인했다. 기존 화면 표시만 개선하고 실제 전투 좌표·사거리·피해·성장 규칙은 유지한다. 향후도 범위/순서/검증 기준을 먼저 제시한다.

2026-09-12 최신 사용자: Base fresh-read 후 벤치마킹·실무조사→구현→검증→개선 루프를 일상적인 별도 승인 없이 계속 수행하도록 위임했다. 이번 적용은 기존 단일 전선 검토판 UI의 실제 공세 예보와 생산 상태 관측이다. 임의의 핵심 게임 의미 변경, 최종 아트/Human 승인, 다른 PR 흡수, main 직접 push 및 Base lock 교체로 확대하지 않는다. 정확한 조사·실행·잔여 작업은 기존 visible-battle-slice 계획이 소유한다.

2026-09-12 ‘계속 진행/작업재개’: 기존 UI/투명 이미지/모션 한정 승인으로 방패병 4자세 분리·공통 피벗·Aseprite 조립·실제 베기 판정을 진행한다. 원화 재생성이 아니라 기존 후보의 손실 없는 분리이며 최종 아트/Human 승인으로 확대하지 않는다. exact 파일과 시간/판정 owner는 APPROVED_REPLAN_UI_MOTION_BUILD_SCOPE_20260911.md 및 front_run.gd다.

2026-09-12 최신 사용자 승인: 로컬 배경 제거 도구 사용. 검증된 불필요 파일은 삭제하지 않고 별도 폴더에 원래 경로/해시/사유와 함께 모아 링크를 제공하며 사용자가 직접 삭제한다. 현재 사용 원본과 불확실한 사용자 파일은 보존한다. 베일 10종의 alpha-only 후보 연결은 현재 UI/투명 이미지 BUILD 범위이며 새 원화·완성 모션·최종 아트 승인으로 확대하지 않는다.

2026-09-11 후속 승인: 사용자 ‘진행해’는 직전 제안한 UI·투명 이미지·모션 범위의 기획 전용 계약 전환을 승인한다. `docs/process/APPROVED_REPLAN_UI_MOTION_BUILD_SCOPE_20260911.md`의 정확한 파일 범위만 허용하며 Base 버전/기준점/Git 보호를 변경하지 않는다. 구현 검토판에 후보를 연결할 수 있지만 생성 이미지 최종 승인 또는 완성 모션 승인으로 확대하지 않는다.

2026-09-11 최신 사용자 승인: ‘인게임에서 전투하는 거, 병종건설도 볼 수 있게. 이미지 연결’. 단일 전선 실전 검토 구간의 BUILD를 개방한다. 아래 기획-only/구현 보류는 이 범위에서 superseded. 실행 계약: `docs/superpowers/plans/2026-09-11-visible-battle-slice.md`. 후보 도감 아틀라스는 검토판의 카드 표현으로만 연결하며 최종 전투 스프라이트/모션 승인·전체 게임 완성을 뜻하지 않는다. 10병종 기본 수치/생산·일반/특수 T1→T2 계열, 1맵 10라운드 골격을 검토한다. T3/등급 스킬/영웅/5맵 완성은 후속이다.

최신 사용자 확정: 일반병종과 특수병종은 별도. 남은 작업 진행 요청은 기획·준비 보완으로 실행하며 전체 최종 구현 승인으로 확대하지 않는다. v3의 별도 일반병영/특수병영과 10병종 분류가 현행 검토안이다. 과거 특수 T1 토큰 규칙 충돌은 본문에 표시하고 추천안/사용자 확정을 구분한다.

최신 사용자 교정: 병종별 아군/베일 이미지, 티어·등급별 스킬, 영웅과 건물 이미지를 Blueprint에 포함한다. 병영(방패병)이 T1이며 건물 업그레이드 후 다른 병종으로 특화한다. 구체 분기·가격·영웅 이름·숙련 획득 규칙은 권장안이며 아직 사용자 확정이 아니다. 현재 Blueprint owner의 군수·등급·영웅 절이 상세안을 소유한다.

2026-09-11 최신 승인 범위: 사람용 Blueprint의 상세 설계·SWOT·벤치마킹·실제 사용 가능한 자산 후보 제작을 위임한다. 예시 십보강호 PDF는 구조 참고만 한다. 아래 이미지 제작 보류는 해제하되 후보 품질 승인과 제품 구현은 최종 사용자 검토 후다. 검토 owner: `docs/design/OMENWARD_HUMAN_BLUEPRINT_REVIEW_20260911.md`. 추천 세부 수치와 생성 후보를 사용자 확정으로 승격하지 않는다.

최신 사용자 확정: 스테이지(맵) 선택 → 10+α 라운드 → 라운드별 복수 웨이브. 라운드는 일정 시간 진행하고 종료 후 재정비. 전체 라운드 생존 또는 적 본진 점령으로 맵 클리어. 상세 규칙은 조사·벤치마킹을 통해 권장안으로 정리하도록 위임했으며 이미지 제작은 보류한다. 상세 owner: `docs/design/OMENWARD_COMMAND_FLOW_AND_MOTION_BLUEPRINT_2026-09-10.md` §1. 추천 수치와 추가 규칙의 runtime/Human 승인까지 의미하지 않는다.

2026-09-10 공격 모션 변경: 사용자 ‘베기 모션으로 하자’. 방패병 기본 공격의 현행 제작 방향은 대각선 내려베기다. 이전 찌르기 pilot은 비교/실패 이력이며 제작 기준에서 제외한다. 모션 방향 승인과 새 프레임 품질·runtime 승인은 구별한다. Blueprint §8의 slash pilot을 따른다.

2026-09-10 후속 진행 승인: 방패병 V3의 품질 방향을 이어서 비인간 베일 pair와 전장 적합성 후보를 제작한다. 새 베일/합성 결과를 미리 승인된 것으로 처리하지 않는다. 상세 후보와 남은 검증은 Blueprint §6.2를 따른다.
...
```text
징조 관측
→ Stage 1: 지휘 거점·전진기지 방어·잠긴 건설 패드 이해 / 3×3 룰렛 학습
