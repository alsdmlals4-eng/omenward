...
플레이어는 **징조수호관(Omen Warden)** 으로서 하나의 전진 전선의 Omen Signature를 읽고, 건물·TokenSource로 동원 확률을 설계한 뒤 3×3 징조륜의 결과를 제한적으로 조작하고 병력을 단일 전선에 비가역 커밋한다. 전투 뒤에는 인과 Review로 다음 설계를 고친다.
...
OPEN_BATTLEFIELD_TOWER_ONLY_LAYOUT = SINGLE_MARCH_FRONT__IMPLEMENTED__MACHINE_VERIFIED
...
- Single-front implementation is present and machine-verified; live runtime and human evidence remain separate pending gates.
