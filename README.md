...
```text
Stage 1 = 예고→첫 건설 선택→룰렛→병력 결과→비가역 배치→실전 전투→복기→첫 상인
...
첫 상인은 Stage 1 정비시간에 등장하며 선택 사항과 골드 기회비용만 가르친다. 첫 건설 후보·최소 유효 경로·Danger/Boss 세부·실패 규칙·정확 시간은 아직 `PENDING_GRILLME`다.
