...
func generate(
	...
	for slot: int in range(maxi(candidate_count, 0)):
		if working.is_empty():
			break
	...
	return {
		"ok": selected.size() == maxi(candidate_count, 0),
		"reason": "OK" if selected.size() == maxi(candidate_count, 0) else "INSUFFICIENT_ELIGIBLE_CANDIDATES",
		"candidates": selected,
		"seed": seed,
