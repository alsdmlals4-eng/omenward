...
func _init() -> void:
	...
	if run.pending_roulette_rewards.is_empty():
		...
		for reward_index in run.pending_roulette_rewards.size():
			_expect(run.assign_pending_reward(reward_index, &"top"), "COMMIT accepts an explicit lane assignment", failures)
		_expect(run.confirm_pending_deployment(), "all assignments apply atomically and enter BATTLE", failures)
...
func _test_reward_commit_path(tutorial: Resource, failures: PackedStringArray) -> void:
	...
	_expect(run.assign_pending_reward(0, &"middle"), "reward COMMIT accepts an irreversible chosen front", failures)
	...
	_expect((run.battle.lanes[&"middle"].units as Array).size() == 1, "reward COMMIT spawns on the chosen front", failures)
