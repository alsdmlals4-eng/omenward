...
class WideConnectedStrategicMapContractTest(unittest.TestCase):
    def test_approved_candidate_promotion_keeps_provenance_and_runtime_ceiling(self) -> None:
        text = CANDIDATE_RECORD.read_text(encoding="utf-8")
        self.assertIn("status: USER_APPROVED__CANON_REGISTERED__IMPLEMENTED__RUNTIME_NOT_RUN", text)
