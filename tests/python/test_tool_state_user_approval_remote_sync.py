...
class ToolStateUserApprovalRemoteSyncTest(unittest.TestCase):
    def test_remote_main_evidence_is_stale_relative_to_user_approved_state(self) -> None:
        project = PROJECT.read_text(encoding="utf-8")
        self.assertIn('enabled=PackedStringArray("res://addons/godot_ai/plugin.cfg")', project)
        self.assertNotIn('res://addons/gut/plugin.cfg', project)
        self.assertNotIn('res://addons/hera_agent_godot/plugin.cfg', project)
        self.assertIn('version="3.1.2"', GODOT_AI.read_text(encoding="utf-8"))
    ...
    def test_authority_records_user_approval_without_claiming_remote_sync(self) -> None:
    ...
    def test_durable_state_replaces_approval_blockers_with_remote_sync_blockers(self) -> None:
    ...
    def test_tool_roles_remain_non_overlapping_and_user_approved(self) -> None:
        ...
        self.assertEqual(tools["higodot"]["remote_version_observed"], "3.1.2")
        self.assertEqual(tools["higodot"]["remote_sync_status"], "REQUIRED")
        ...
        self.assertFalse(tools["gut"]["remote_editor_plugin_enabled"])
        ...
        self.assertFalse(tools["hera"]["remote_editor_plugin_enabled"])
