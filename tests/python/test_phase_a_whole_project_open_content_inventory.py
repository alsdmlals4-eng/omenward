...
# Thin-adapter rule: phase-state literals belong in entry/current planning routers,
# not duplicated across the gameplay GDD, implementation ledger, core, and workbook.
CURRENT_ROUTERS = (
    ROOT / "AGENTS.md",
...
class PhaseAWholeProjectOpenContentInventoryTest(unittest.TestCase):
    def test_v45_workflow_executes_inventory_contract(self):
        text = WORKFLOW.read_text(encoding="utf-8")
        self.assertIn("tests/python/test_phase_a_whole_project_open_content_inventory.py", text)
        self.assertIn("python -m unittest tests.python.test_phase_a_whole_project_open_content_inventory -v", text)
    def test_review_artifact_exists_and_has_additive_phase_model(self):
        self.assertTrue(REVIEW.is_file())
        text = REVIEW.read_text(encoding="utf-8")
        for marker in STATE_MARKERS + OPEN_GROUPS: self.assertIn(marker, text)
    def test_entry_routers_do_not_equate_onboarding_10_of_10_with_whole_project_completion(self):
        for path in CURRENT_ROUTERS:
            text = path.read_text(encoding="utf-8")
            for marker in STATE_MARKERS: self.assertIn(marker, text, f"{path.relative_to(ROOT)} missing {marker}")
            self.assertNotIn("WHOLE_PROJECT_PLANNING_COMPLETE = TRUE", text)
    def test_only_three_genuine_new_product_decision_groups_are_identified(self):
        review = REVIEW.read_text(encoding="utf-8")
        for marker in OPEN_GROUPS: self.assertIn(marker, review)
        self.assertIn("Genuine new product Decision groups = 3", review)
    def test_held_hero_and_meta_are_not_current_implementation_input(self):
        review = REVIEW.read_text(encoding="utf-8")
    ...
    def test_building_t3_is_partial_open_but_troop_t3_structure_is_not_reset(self):
        review = REVIEW.read_text(encoding="utf-8")
    ...
    def test_prior_non_blocker_classification_is_preserved(self):
        review = REVIEW.read_text(encoding="utf-8")
    ...
    def test_next_step_is_user_grill_me_not_automatic_product_approval(self):
        review = REVIEW.read_text(encoding="utf-8")
