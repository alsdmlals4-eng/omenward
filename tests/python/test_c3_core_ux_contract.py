...
class C3CoreUxContractTests(unittest.TestCase):
    """Protect runtime contracts plus the current v0.26 V2 documentation boundary."""
    ...
    def test_token_ledger_removal_is_rejected(self) -> None:
        errors = self._errors_after(
            "scripts/core/core_ux_service.gd", '"token_ledger"', '"ledger_removed"'
        )
    ...
    def test_read_only_headless_regression_removal_is_rejected(self) -> None:
        ...
        errors = self._errors_after(
            "tests/headless/c3_core_ux_test.gd", phrase, "removed read-only check"
        )
    ...
    def test_boundary_regression_removal_is_rejected(self) -> None:
        ...
        errors = self._errors_after(
            "tests/headless/c3_core_ux_test.gd", phrase, "removed boundary check"
        )
    ...
    def test_missing_staged_omen_regression_is_rejected(self) -> None:
        errors = self._errors_after(
            "scripts/waves/wave_director.gd", "OMEN_T5_SECONDS", "OMEN_LAST_SECONDS"
        )
    ...
    def test_current_v2_audit_proof_regression_is_rejected(self) -> None:
        errors = self._errors_after(
    ...
    def test_current_v2_readme_boundary_loss_is_rejected(self) -> None:
        errors = self._errors_after(
            "README.md", "LEGACY_C1_C2_C3_PROVEN", "LEGACY_PROOF_REMOVED"
        )
        self.assertTrue(any("README.md" in error and "LEGACY_C1_C2_C3_PROVEN" in error for error in errors))

    def test_current_v2_status_boundary_loss_is_rejected(self) -> None:
        errors = self._errors_after(
    ...
    def test_current_v2_product_authority_loss_is_rejected(self) -> None:
        errors = self._errors_after(
            ...
            "PRODUCT_CODE_AUTHORITY_REMOVED",
        )
        self.assertTrue(any("PRODUCT_CODE_NOT_AUTHORIZED" in error for error in errors))

    def test_current_v2_roadmap_baseline_loss_is_rejected(self) -> None:
        errors = self._errors_after(
            "docs/OMENWARD_ROADMAP.md",
            "기존 기술 기준선·C1·C2·C3 자동 증거 확보",
            "legacy evidence removed",
        )
        self.assertTrue(any("기존 기술 기준선" in error for error in errors))
