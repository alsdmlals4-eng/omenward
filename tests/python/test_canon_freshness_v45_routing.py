...
class CanonFreshnessV45RoutingTest(unittest.TestCase):
    ...
    def test_current_routers_use_v48_and_current_visual_closeout(self) -> None:
        ...
        self.assertIn("PROJECT_STATE = PAUSED_AT_BATCH_EDGE_CLEANUP_APPROVAL_GATE", decisions)
        ...
        self.assertIn("status: PAUSED_AT_BATCH_EDGE_CLEANUP_APPROVAL_GATE", active)
        self.assertIn("CURRENT_APPROVED_REPLAN_DECISIONS = 21", active)
        self.assertIn("CURRENT_NEXT = USER_EXPLICIT_BATCH_EDGE_CLEANUP_AND_MASTER_EXPORT_APPROVAL", active)
