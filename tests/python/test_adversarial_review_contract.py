...
class AdversarialReviewContractTests(unittest.TestCase):
    ...
    def test_shared_contract_blocks_false_completion(self) -> None:
        ...
        self.assertIn("미해결 결함", text)
        self.assertIn("NOT_RUN", text)
        self.assertIn("PASS는 실행한 해당 검사에만", text)
    ...
    def test_shared_contract_matches_registry_routing_limits(self) -> None:
        from tools.route_skills import load_registry, route
        current = load_registry(ROOT / 'skills/SKILL_REGISTRY.json')
        result = route('검토', current, forced_mode='REVIEW')
