...
def validate(root: pathlib.Path) -> list[str]:
    ...
    if docs:
        ...
        _require_count(
            ...
            '- "!docs/base/**"',
            2,
            "documentation workflow must exclude Skill base docs from PR and push triggers",
            errors,
        )
        _require_count(
            docs,
            ...
            2,
            "documentation workflow must exclude the Skill base version document from PR and push triggers",
            errors,
        )
        _reject(
            docs,
            '.github/workflows/validate-omenward-core.yml',
            "documentation workflow must not duplicate core workflow changes",
            errors,
        )
        _reject(
            docs,
            '.github/workflows/validate-skill-system.yml',
            "documentation workflow must not duplicate Skill workflow changes",
            errors,
        )
    ...
    if core:
        ...
        _require(
            ...
            "core PR job must validate the CI usage contract",
            errors,
        )
    ...
    if skill:
        ...
        _require(
            skill,
            "python tools/validate_ci_usage_contract.py",
            "Skill workflow must validate the CI usage contract",
            errors,
        )
        _require(
            skill,
            "tests.python.test_ci_usage_contract",
            "Skill workflow must run CI usage mutation tests",
            errors,
        )
