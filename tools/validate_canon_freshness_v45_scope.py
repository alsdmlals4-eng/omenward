...
HISTORICAL_V44_AUTHORITY = {"docs/process/ACTIVE_INTEGRATED_CONTRACT_BINDING_2026-08-06.md", "docs/operations/ACTIVE_INTEGRATED_CONTRACT_STATE.v1.json"}
...
POSTMERGE_CI_ALLOWED_FILES = {".github/workflows/validate-omenward-core.yml", "tests/python/test_ci_usage_contract.py", "tools/validate_ci_usage_contract.py", "tests/python/test_canon_freshness_v45_scope.py", "tools/validate_canon_freshness_v45_scope.py"}
...
CURRENT_CONSUMER_RECONCILIATION_ALLOWED_FILES = {"docs/ACTIVE_CONTEXT.md", "docs/CURRENT_IMPLEMENTATION_STATUS.md", "docs/DECISIONS_PENDING.md", "tests/python/test_canon_freshness_v45_routing.py", "tests/python/test_canon_freshness_v45_scope.py", "tools/validate_canon_freshness_v45_scope.py"}
...
CONTENT_CLOSURE_BENCHMARK_FIRST_ALLOWED_FILES = {
    ".github/workflows/validate-canon-freshness-v4-5.yml", "AGENTS.md", "docs/ACTIVE_CONTEXT.md", "docs/DECISIONS_PENDING.md", "docs/design/APPROVED_OMENWARD_WHOLE_PROJECT_CONTENT_CLOSURE_2026-08-11.md", "docs/process/APPROVED_OMENWARD_BENCHMARK_INDUSTRY_RESEARCH_FIRST_2026-08-11.md", "docs/superpowers/plans/2026-08-11-content-closure-benchmark-first.md", "tests/python/test_content_closure_benchmark_first.py", "tests/python/test_canon_freshness_v45_scope.py", "tools/validate_canon_freshness_v45_scope.py",
}
...
def validate_canon_freshness_scope(changed_files: Iterable[str]) -> list[str]:
    ...
    if historical: errors.append(f"historical v4.4 authority mutation is forbidden: {historical}")
    protected = sorted(p for p in changed if _is_protected_product(p))
    if protected: errors.append(f"protected product paths are forbidden in v4.5 canon freshness scope: {protected}")
    unexpected = sorted(changed - APPROVED_FILES)
    if unexpected: errors.append(f"v4.5 canon freshness transition contains unapproved files: {unexpected}")
    ...
    for allowed, required, label in modes:
        if changed <= allowed: return _validate_required(changed, required, label)
...
def changed_files_from_git(base: str, head: str) -> list[str]:
    ...
    if result.returncode: raise RuntimeError(result.stderr.strip() or "git diff failed")
...
def main() -> int:
    ...
    args = parser.parse_args(); changed = changed_files_from_git(args.base, args.head)
