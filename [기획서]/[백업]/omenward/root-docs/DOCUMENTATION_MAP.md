...
> 현행 구조 진입점은 `[기획서]/00_프로젝트_허브/START_HERE.md`다. 이 문서는 기존 경로의 세부 부록·이력 라우터로 유지한다.
...
→ 관련 APPROVED 책임 문서
→ 시각 작업이면 ../../06_아트/증거/VISUAL_REFERENCE_INDEX.md
→ OMENWARD_ROADMAP.md
...
- `../../06_아트/증거/VISUAL_REFERENCE_INDEX.md`는 이미지의 승인·부분 참고·폐기 상태와 올바른 해석을 관리한다.
