...
> **2026-09-16 최신 사용자 변경:** 신규 원정은 3전선 동시 진행으로 복귀한다. 기본 화면은 공통 양측 본진과 3개 경로의 간단한 전황 미니맵이며, 선택한 전선만 기존 캐릭터 전투 화면으로 확대한다. 구형 저장은 단일 전선 규칙/화면을 유지한다. 상세 책임 owner는 current Blueprint와 Decisions다. 월간 작업일지는 기존 미제출 v0.2 PDF에 날짜별 요약을 누적한다. 작업마다 새 증빙집을 만들지 않으며, 이미 제출한 문서의 정정 이력 보호는 유지한다.

> **2026-09-14 사용자 운영 추가:** 앞으로 분리 오브젝트 이미지는 단색 크로마키 배경으로 생성한 뒤 배경을 제거한다. 대상 색과 겹치지 않는 키색을 선택하고 원화 보존, 실제 RGBA·가장자리·모션 상태·runtime consumer 검증을 유지한다. 완성 배경화의 내용까지 제거하지 않는다. 월간 「OMENWARD AI 활용 작업일지·증빙집」은 블루프린트와 별도인 파생 보고서이며 `C:/Users/user/Documents/증빙서류/9월 증빙서류`에 발행한다. 날짜·프롬프트·결과·검증의 원본 연결을 보존하고 실제 작업일/기록일/캡처일/발행일, 사후 정리/미확인, AI 계정/결제 미제공을 분리한다. PDF 자체를 시간 인증·지원 적격 증명·공식 정산 양식으로 표현하지 않는다. 제출본은 덮어쓰지 않고 정정 이유를 포함한 새 버전을 발행한다. 현재 사용자 제공 협약 설명은 원본 확인 증거가 아니다.
