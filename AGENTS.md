...
## 10. 승인된 whole-project content closure + 작업 전 benchmark gate
...
앞으로 모든 비사소 OMENWARD 작업은 fresh Base/project/Sheet read 뒤 **관련 벤치마킹·현업조사 → `ADOPT / ADAPT / AVOID / TEST / IGNORE` 분류 → canon 충돌 확인**을 먼저 수행하고 나서 작업한다. 외부 사례는 자동 authority가 아니다.

현재 장르 shorthand는 `ROGUELITE_STRATEGY_AUTO_BATTLER + ROULETTE_PROBABILITY_BUILDER`이며, 사용자용 표현은 **“룰렛을 설계해 군대를 만드는 로그라이트 전략 오토배틀러”**다.
