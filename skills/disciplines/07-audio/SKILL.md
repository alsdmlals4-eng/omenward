---
name: omenward-audio
description: Omenward의 사운드 책임 원본·구현·검증을 갱신할 때 자동 선택한다.
---
...
Trigger: `audio`, `music`, `sfx`, `voice`, `mixing`

**Use when:** 음악·효과음·보이스·이벤트·믹싱·오디오 피드백을 변경하거나 검수할 때.

**Do not use when:** 시각 자산·코드가 바뀌지만 오디오 이벤트·출력에 영향이 없을 때.

이 Skill은 프로젝트 주 책임 분야 Skill이며 동시에 최대 1개를 기본 선택한다. 필요한 Foundation/Specialist Skill만 별도로 조합한다.

## Canonical source

- Responsibility source: `[기획서]/07_사운드/07_사운드_본책.md`
...
이벤트 이름·재생 조건·우선순위·중복 방지·믹싱 검증을 책임 원본과 구현에 연결한다.
