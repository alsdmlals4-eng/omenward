---
name: omenward-art
description: Omenward의 아트 책임 원본·구현·검증을 갱신할 때 자동 선택한다.
---
...
Trigger: `art`, `visual-direction`, `sprite`, `battlefield-concept`, `palette`

**Use when:** 아트 방향·유닛 시각 형식·전장 이미지·팔레트·승인 자산을 변경하거나 검수할 때.

**Do not use when:** 테스트 실행이나 수치 수정만 하며 시각 결과에 영향이 없을 때.

이 Skill은 프로젝트 주 책임 분야 Skill이며 동시에 최대 1개를 기본 선택한다. 필요한 Foundation/Specialist Skill만 별도로 조합한다.

## Canonical source

- Responsibility source: `[기획서]/06_아트/06_아트_본책.md`
...
승인 전장 시안과 Asset Registry 해시를 보존하고 생성·편집 결과는 사용자 승인 전 정본으로 승격하지 않는다.
