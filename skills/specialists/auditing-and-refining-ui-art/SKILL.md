...
description: Omenward에서 Godot·Web UI 결과를 구조·가독성·일관성·피드백·마감 영역으로 감사하고 승인 범위만 개선한다.
---
...
- Source skill: `Base:skills/auditing-and-refining-ui-art/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `ui-art-audit`, `godot-ui`, `web-ui`, `visual-refinement`, `render-review`

**Use when:** Godot·Web UI 결과를 구조·가독성·일관성·피드백·마감 영역으로 감사하고 승인 범위만 개선한다.

**Do not use when:** UI 결과 감사 없이 새 이미지 프롬프트나 기술 카드만 설계하는 경우.
...
960×540 논리 해상도, 1920×1080 기준, viewport/keep/integer/nearest 계약을 보호한다. 1920×1080·1280×720 사람 시각 QA가 없으면 [미검증]이다.
