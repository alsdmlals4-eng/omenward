...
description: Omenward에서 대표 플레이 구간으로 핵심 재미·목표 품질·접근성·성능·제작 파이프라인을 함께 증명한다.
---
...
- Source skill: `Base:skills/designing-vertical-slices/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `vertical-slice`, `first-playable`, `production-proof`, `quality-bar`, `pipeline-proof`, `playtest-evidence`, `accessibility-target`, `performance-budget`

**Use when:** 대표 플레이 구간으로 핵심 재미·목표 품질·접근성·성능·제작 파이프라인을 함께 증명한다.

**Do not use when:** 단일 기능, 가장 위험한 가설만 보는 작은 Prototype, 전체 MVP 기능 나열.
...
현재 수직 슬라이스의 게임 규칙·콘텐츠 재설계는 별도 승인 전 금지다. 기존 플레이 가능한 경로를 기준으로 대표성·품질·파이프라인·실제 플레이 증거를 분리한다.
