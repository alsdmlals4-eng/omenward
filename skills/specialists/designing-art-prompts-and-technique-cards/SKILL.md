...
description: Omenward에서 아트·UI 제작 기술과 이미지 생성·편집 프롬프트를 설계한다.
---
...
- Source skill: `Base:skills/designing-art-prompts-and-technique-cards/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `art-direction`, `image-prompt`, `technique-card`

**Use when:** 아트·UI 제작 기술과 이미지 생성·편집 프롬프트를 설계한다.

**Do not use when:** 기존 승인 이미지의 상태·경로 정리만 필요한 경우.
...
승인 전장 시안과 Asset Registry 해시를 보호하고, 생성 프롬프트는 목표·구도·스타일·금지 요소·후처리·검수 기준을 분리한다. 생성 결과는 승인 전 정본 자산이 아니다.
