...
description: Omenward에서 정본·경로·ID·Schema·정책·생성기 변경이 참조·템플릿·테스트·파생본에 빠짐없이 전파됐는지 감사한다.
---
...
- Source skill: `Base:skills/auditing-canonical-reference-freshness/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `canonical-source-change`, `stale-reference`, `rename`, `path-migration`, `schema-change`, `skill-id-change`, `document-id-change`, `generator-change`, `publication-stale`, `propagation-gap`

**Use when:** 정본·경로·ID·Schema·정책·생성기 변경이 참조·템플릿·테스트·파생본에 빠짐없이 전파됐는지 감사한다.

**Do not use when:** 외부 참조가 없는 L0 오탈자 또는 동일 입력 검사 재실행.
...
Base 기준 커밋, Registry 필드, Skill 경로, PDF Manifest, Issue·PR 설명의 오래된 참조를 함께 검사한다. PR #18 전용 d2457e7은 legacy_extension 문맥 외 활성 기준으로 남으면 FAIL이다.
