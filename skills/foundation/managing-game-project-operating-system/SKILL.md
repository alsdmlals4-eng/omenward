...
description: Omenward에서 운영체계를 설치·감사·구형 산출물 정리·승인 마이그레이션·전체 연결 검수할 때 사용한다.
---
...
- Source skill: `Base:skills/managing-game-project-operating-system/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `new-project`, `existing-project`, `safe-migration`, `document-restructure`, `operating-system-health`, `legacy-file`, `stale-copy`, `cold-start-failure`

**Use when:** 운영체계를 설치·감사·구형 산출물 정리·승인 마이그레이션·전체 연결 검수할 때 사용한다.

**Do not use when:** 운영체계와 무관한 작은 구현 또는 현행 정본과 참조가 명확한 단일 파일 편집.
...
Omenward는 기존 프로젝트이므로 기본은 audit→승인→migrate/reconcile-legacy→verify다. 기존 승인 결정·수치·자산·보류·실패는 조사와 승인 없이 삭제하지 않는다.
