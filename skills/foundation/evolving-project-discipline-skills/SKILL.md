...
description: Omenward에서 Foundation·Specialist·분야 Skill을 생성·통합·학습·갱신하고 오래된 ID·경로를 정리한다.
---
...
- Source skill: `Base:skills/evolving-project-discipline-skills/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `skill-audit`, `skill-learning`, `skill-consolidation`, `missing-discipline-skill`

**Use when:** Foundation·Specialist·분야 Skill을 생성·통합·학습·갱신하고 오래된 ID·경로를 정리한다.

**Do not use when:** Skill 구조나 호출 조건에 영향 없는 일반 구현 작업.
...
24개 활성 진입 Skill은 Registry와 실제 경로가 1:1이어야 한다. 실패·중요 결정·재사용 교훈·검증 결과만 Learning Log에 기록하며 단순 작업 로그는 남기지 않는다.
