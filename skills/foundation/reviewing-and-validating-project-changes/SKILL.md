...
description: Omenward에서 코드·데이터·문서·자산 변경을 승인 계약, 정본·참조, 실행 결과와 회귀 증거로 통합 검증한다.
---
...
- Source skill: `Base:skills/reviewing-and-validating-project-changes/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `change-review`, `contract-check`, `diff-review`, `reference-freshness`, `static-validation`, `runtime-validation`, `accessibility-review`, `performance-profile`, `regression`, `evidence-report`

**Use when:** 코드·데이터·문서·자산 변경을 승인 계약, 정본·참조, 실행 결과와 회귀 증거로 통합 검증한다.

**Do not use when:** 변경 없는 아이디어 비교, 전문 UI 시각 감사만 필요한 경우, 같은 입력의 검사 재실행.
...
Godot editor import→headless 6종→runtime smoke→문서 발행·링크·Registry 검증 순서를 사용한다. 미실행 검사는 PASS로 쓰지 않고 NOT_RUN 또는 [미검증]으로 기록한다.
