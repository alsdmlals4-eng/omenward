...
description: Omenward에서 프로젝트 교훈을 공용 원리로 분리해 Base 제안으로 제출·검토하고 승인 범위만 별도 PR로 구현한다.
---
...
- Source skill: `Base:skills/managing-base-change-proposals/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `project-learning`, `base-promotion`, `repeated-pattern`, `base-change-proposal`, `approved-base-change`

**Use when:** 프로젝트 교훈을 공용 원리로 분리해 Base 제안으로 제출·검토하고 승인 범위만 별도 PR로 구현한다.

**Do not use when:** 프로젝트 고유 수치·세계관·자산뿐이거나 사용자 승인 없이 Base 활성 경로를 직접 변경하려는 경우.
...
Omenward 고유 규칙은 프로젝트에 남긴다. 공용화 후보는 관찰·반례·적용 조건·롤백·검증을 포함하고 제안 PR과 구현 PR을 분리한다.
