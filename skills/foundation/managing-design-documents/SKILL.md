...
description: Omenward에서 등록된 기획 책임 원본을 작성·갱신·재구조화하고 정책에 따라 PDF·Manifest를 발행한다.
---
...
- Source skill: `Base:skills/managing-design-documents/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `design-document`, `responsibility-map`, `roadmap-structure`, `pdf-publication`, `publication-stale`, `document-restructure`

**Use when:** 등록된 기획 책임 원본을 작성·갱신·재구조화하고 정책에 따라 PDF·Manifest를 발행한다.

**Do not use when:** 문서 구조·내용·발행 상태에 영향 없는 코드 수정 또는 CURRENT 발행본 단순 조회.
...
서술 책임 원본은 Markdown, Registry·Manifest·상태·ID·경로·게임 데이터는 JSON이다. 본책은 always_sync이므로 원본·PDF·Manifest를 같은 작업에서 갱신한다.
