...
description: Omenward에서 현재 상태·다음 작업·위험·미검증을 새 채팅과 담당자에게 압축 전달한다.
---
...
- Source skill: `Base:skills/maintaining-project-context-and-handoff/SKILL.md`
- Installation: Base 공용 Skill을 그대로 복제한 파일이 아니라 Omenward의 경로·게이트·검증 계약에 맞춘 로컬 진입 어댑터다.

## Automatic routing

Trigger: `handoff`, `active-context-change`, `phase-boundary`, `new-chat`

**Use when:** 현재 상태·다음 작업·위험·미검증을 새 채팅과 담당자에게 압축 전달한다.

**Do not use when:** 저장소 상태가 바뀌지 않은 단순 대화.
...
ACTIVE_CONTEXT.md가 현재 상태의 기본 원본이다. Handoff는 경계 시점 스냅샷이며 두 번째 활성 상태 원본이 아니다. 브랜치·PR·검증 결과를 실제 상태와 맞춘다.
